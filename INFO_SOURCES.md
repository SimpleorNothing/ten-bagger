...
> 최종 갱신: 2026-07-12 · 기준 메뉴: 6탭 (Phase 2g + 01 카드 렌즈 + 뉴스 스크리닝 + 03 관점과 정보 얻기)
...
> 인테이크(자료 → 관점 추출) · 선별(사람 채택) · 반영(채택분만 타 메뉴 스트립)을 분리한 뷰.
> **narrative ≠ numbers**: 내러티브 route 는 `signal_log` 까지만(클라 clamp), 숫자 파일(earnings·judgment·gamma·holdings)은 **자동 변경 없음** — '반영 대기'로만 04 에 표시.

| 메뉴 | 정보명 | 자동/수동 | 업데이트 주기 | 정보 소스 |
|---|---|---|---|---|
| 관점과 정보 | 관점 추출 (리포트·기사·유튜브 → 요약·claims·noise·스틸맨) | 자동 (요청 시) | 인테이크 실행 시 | worker `/api/insight` → Claude (본문 없으면 web_search) |
| 관점과 정보 | PDF·TXT 본문 추출 | 자동 (클라) | 첨부 시 | `insight.js` + pdf.js CDN 지연로드 (최대 40p) |
| 관점과 정보 | 채택한 관점 (반영 대기/완료) | 수동 (사람 체크가 게이트) | 채택 시 | worker `/api/insights` (R2) + localStorage 캐시 |
| 관점과 정보 | 반영 스트립 (01·02·04·05·시그널로그) | 자동 (채택분 파생) | 채택 시 | `insight.js` route 라우팅 (숫자 route 는 대기 배지만) |

- UI·로직 = `insight.js` + `insight.css` (루트 정적 에셋, 자가 마운트). `index.html` 은 스크립트 태그 1줄만 참조.
- 채택 기본 체크(추천) = route≠none AND N+I+C ≥ 4 AND impact ≥ 1. 추천은 제안일 뿐, **저장은 사람이 체크한 것만**.

---

## 04 리밸런싱 (`v-decision` — 결정보드 + 시장 모멘텀 전망 + 방향 확률 추정 · `v-port`/`v-tracker`/`v-macro`는 2026-07-11 재편으로 뷰서 제외·코드 잔존)
...
9. **관점 반영 루프(2026-07-12)** — 03 채택 관점 중 숫자 route 는 **수기 반영**(파일 편집 후 03 에서 '반영 완료' 표시). 자동화 없음(사람 승인 게이트).
...
- 2026-07-12 · **03 관점과 정보 얻기 신설**(6탭: 리밸런싱 04·캘린더 05·메모 06). 인테이크 → 선별(claims 체크·narrative clamp) → 반영(채택분만 스트립, 숫자는 '반영 대기'). worker `/api/insight`·`/api/insights`(R2) + `insight.js`/`insight.css` 자가 마운트. index.html 변경은 스크립트 태그 1줄(패치 460B) — 대형 b64 전사 손상 회피.
