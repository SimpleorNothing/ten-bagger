...
- `/api/council-discussions`는 **최근 생성 결과 1건만** R2에 보관한다(2026-08-01). 새 토론 저장은 기존 이력을 교체하고, 기존에 누적된 여러 건은 첫 조회에서 생성 시각 기준 최신 1건만 남도록 정리한다. 삭제·음성 재생은 그 한 건에 그대로 적용한다.
