...
| ↳ 자료 입력 경로 = URL · 본문 붙여넣기 · 파일(PDF·TXT) · **이미지 OCR** | 수동 | 작성 시 | 드롭존/파일선택이 이미지도 수용 + 텍스트칸·드롭존에 **캡처 이미지 붙여넣기(Ctrl/⌘+V)** → `tesseract.js@5`(kor+eng, CDN 지연 로드·워커 1회 생성 재사용) 로 글자 인식해 위 칸에 append. 클라 전용(서버 무변경) |
...
- **뉴스 = 1일 2회(2026-07-12~).** `06:12 KST`(미국 장 마감 반영) · `18:12 KST`(한국 장 마감 반영). 각 세션에 30분 후행 + 1시간 백업 트리거(스케줄러 누락 흡수). **가드 6h < 세션 간격 12h** → 같은 세션의 중복만 스킵되고 두 세션은 각각 실행. `workflow_dispatch`는 가드 우회. ✅ **2026-07-16 저녁 세션 cron 설치 완료** — 라이브는 이제 아침·저녁 1일 2회.
...
- 2026-07-16 22:43 · **날짜 없는 실적 발표 「일정 공시」 m=0 컷 규율 신설(MV 2→3).** SimpleorNothing 지시 — 「실적 발표 일정을 공시했다」만 있고 정작 **언제 발표하는지(월·일)가 없으면 기사로서 의미 0** → 표시 제외. 근인: 파이프라인이 **제목만** LLM에 넘겨 본문의 발표일이 요약에 못 담기는 구조(`summarizeArticles` 입력=제목). `news-screen.mjs`에 `RE_ERN_SCHED`·`RE_HAS_DATE`·`isSchedNoDate` 추가 → `ruleM`(title+`a`)에 컷 + LADDER에 룰 명시. MV 3으로 올려 과거분 자동 재채점(요약 `a·w` 재사용) → 기존 기사는 한글 요약이 있어 컷 확정. 실측 리플레이(archive 835건): 무날짜 일정 공시 **5건 컷**(VRT·PLTR·LITE·AMD·ALAB), 날짜 있는 건 유지(QCOM 7/29·ALAB 8/4·VRT 7/29 확정판), m=2 펀더멘털·프리뷰(VICR) 오탐 0건. narrative≠numbers 불변(표시 창만·숫자 파일 무변). §3 물질성 사다리 m=0 행·3층 판정. **데이터 반영은 다음 뉴스 크론(06:12/18:12) 또는 `Update news feed` 수동 dispatch부터**(§8-4).
...
- 2026-07-14 23:40 · **시그널 로그 → 03으로 이관.** 구 `#v-siglog` 는 7/12 6탭 재편 때 nav 탭을 잃어 **도달 불가한 고아 뷰**였다. 03 「채택한 관점」 각 관점 아래로 중첩. §3 03·보조데이터 · §8-12.
- 2026-07-14 22:57 · **관련 기사(매크로)를 종목 뉴스와 같은 「두 점」 형식으로 통일.** `fetch-news.mjs`에 `summarizeMacro()` 추가 — MACRO 기사별 두 점(a·w) 증분 생성. 매크로는 m 미적용. §3 01 관련 기사 행 · STYLE_GUIDE §6-5.
- 2026-07-14 · **01 시장 모니터링 전 지표 1일 2회 확정.** SimpleorNothing 지시. 시세·지수·스파크라인(저녁 18:37)·뉴스(저녁 18:12) 추가. §3·§4·§8-11.
- 2026-07-14 · **코스피 스테일 사고 수리.** 7/13 코스피 −8.95% 당일 Yahoo 일봉 누락 → `fetch-prices.mjs`(meta 강제·union·warn)·`fetch-signals.mjs`(KR 종가 백스톱). §8-9·10.
- 2026-07-12 · **뉴스 = 시그널 관측기로 재정의(MV=2)** · 소스 티어·수집 3축·`mv` 세대 · 스크리너 `news-screen.mjs` 분리.
- 2026-07-12 · **v3 — 2문서 체계 확정** · `INFO_SOURCES.md` §3으로 흡수 · §0 시작 프로토콜·§1 불변 규율·§6 GitHub 운영·§7 문서 자기갱신 매핑표 신설.
- 2026-07-11 · 02 답-먼저 재편 · 03 리밸런싱 보드 신설 · 01 Phase 2a~2g.
