...
| 정보명 | 자동/수동 | 주기 | 소스 |
|---|---|---|---|
| **텍스트 브리핑** (결론 → 매크로 게이트 보드 3중 AND → 레이어 갭 표 → 오늘 볼 것 → 액션 아이템 → **스틸맨 반론**) | 자동(열 때 생성·R2 날짜 캐시) | 하루 1회분 | `GET /api/brief?part=0` (worker `handleBrief` · `BRIEF_TEXT_SYS`). 대담(part 1·2)과 **같은 라이브 입력**(`signals`·`gamma`·`holdings`·`calendar`·`signal_log`·`judgment`)을 쓰되 낭독 대본이 아니라 **훑어보는 구조**로 낸다. `gate[].s`는 충족/미충족만 · `layers[].state`는 오버/언더/적정 · `actions`는 전부 조건부 AND |
| **오늘 브리핑 듣기** (2인 대담 · 약 8분) | 자동(열 때 생성·R2 캐시) | 하루 1회분 | 「▶ 오늘 브리핑 듣기」 → `part=1` 먼저 재생 · `part=2`는 재생 중 수신·이어붙임(§ 외부 채널 동일 엔드포인트 재사용). 낭독 = 브라우저 TTS 2보이스 · 말풍선 클릭 = 그 대목만 재낭독 · 배속·음소거 |
...
| ↳ 「다시 만들기」 | 수동 | 필요 시 | `?regen=1` — 그날치를 새 라이브 값으로 덮어쓴다(텍스트만 재생성 · 대담은 다음 열람 때) |

> **저장 시점 = 첫 열람 또는 크론 워밍.** 06 회차는 첫 열람 때 워커가 생성해 저장하는데, 아침에 아무도 안 열면 그날 회차가 빈다(「지난 호」 결번). 이를 막으려고 **`daily-brief-slack.mjs`(07:45 크론)가 직접 게이트에 로그인(`/__auth`)해 `/api/brief?part=0` 을 선호출**, 오늘 회차를 미리 굽는다(`warmBrief()`). 게이트 통과에 리포 시크릿 **`SITE_PASSWORD`** 가 필요하고(워커에 설정된 값과 동일), 이를 **워크플로 `env` 로 스크립트에 넘겨줘야** 동작한다(`.github/workflows/` 편집은 App 403 → 운영자 수동 · §8). 기본은 **텍스트 p0만** 워밍(결번 방지 최소분·어차피 첫 열람 때 생길 생성을 당길 뿐이라 추가 비용 ~0). 팟캐스트 p1·p2까지 미리 구우려면 `BRIEF_WARM_PODCAST=1`. 시크릿·env 가 없으면 워밍은 조용히 건너뛰고 슬랙 본문·링크는 그대로 나간다.
...
| **데일리 브리핑 팟캐스트 (2인 대담 · 약 8분)**(2026-07-19 신설) | 자동(열 때 생성·R2 날짜 캐시) | 하루 1회분(첫 접속 시 생성) | 단독 페이지 **`brief.html`** → `GET /api/brief?part=1\|2` (worker `handleBrief`). 라이브 `signals`·`gamma`·`holdings`·`calendar`·`signal_log`·`judgment`를 워커가 읽어 **진행자(`host`)+애널리스트(`ana`) 2인 대담 대본**을 Claude(opus-4-8)로 생성 → R2 `brief_{YYYY-MM-DD}_p{1,2}.json` 캐시. **파트 분할 이유** = 8분치를 비스트리밍 1회로 뽑으면 `api.anthropic.com` ~100s 한도에 걸린다 → part1(게이트·레이어) 먼저 반환해 재생을 시작하고 part2(종목·액션·**스틸맨**)는 재생 중 뒤에서 받아 이어붙인다. 낭독 = **브라우저 TTS**(ko-KR 음성 품질 점수순 2인 배정 · **「The Energetic Co-Host」 톤** = 진행자 rate 1.12·pitch 1.14 / 애널리스트 rate 1.06·pitch 1.0 · 피치 항상 적용해 화자 분리). `?d=YYYY-MM-DD` 과거분·`?regen=1` 재생성 |
| ↳ **오디오(MP3) 첨부판 = 미가동(제안본)** | — | — | `scripts/proposed-workflows/daily-brief-podcast-audio.{yml,md}` + `brief-tts.mjs`. Gemini 멀티스피커 TTS(**`gemini-3.1-flash-tts-preview` · AI Studio 「The Energetic Co-Host」 팟캐스트 톤 · 보이스 Puck/Kore** · 모델·보이스·스타일 전부 env 오버라이드) → MP3 → 슬랙 `files.uploadV2`. **선행 3건 전부 운영자 수동**(§8) |
...
- **모닝 브리핑 크론 워밍 — 스크립트 배선 완료·워크플로 env 선행(2026-07-20)**: `daily-brief-slack.mjs`에 `warmBrief()`(게이트 로그인 → `/api/brief?part=0` 선호출) 배선 완료. `SITE_PASSWORD` 리포 시크릿은 추가됨. **남은 1건 = 운영자 수동**: `.github/workflows/daily-brief-slack.yml` 의 `run: node scripts/daily-brief-slack.mjs` 스텝 `env:` 에 `SITE_PASSWORD: ${{ secrets.SITE_PASSWORD }}` 한 줄 추가(App workflows-write 403이라 스크립트 PR로는 못 넣는다). 그 줄이 들어가야 러너 `process.env.SITE_PASSWORD` 가 채워져 워밍이 실제로 돈다. 그 전까지 `warmBrief()`는 조용히 건너뛴다(슬랙 본문·링크는 정상). 팟캐스트 p1·p2 사전 생성은 `BRIEF_WARM_PODCAST=1`(선택 env).
