...
| 정보명 | 자동/수동 | 주기 | 소스 |
|---|---|---|---|
| **AI 수요·공급 로드맵** (02 맨위 · 판정 보드[수요 가속/공급 추격/주가 여력]+진화 4단계+CAPEX 리비전 트랙+수요 4사/공급 3사·중국 연도 매트릭스·업체 클릭 상세) | 수동 | 분기(컨센서스 캡처 시) | `aisd.js` 자가 마운트(#dsAisd · flags.js 패턴 — worker.js가 `<script defer>` 주입 · v-thread 최상단). 전역 토큰만·`ds-*` 스코프·신규 :root 토큰 0. 수치=컨센서스 방향성 추정, **리비전 트랙은 캡처 축적 전 예시 표시**. narrative 층 — 숫자 파일 무관. 재판정 트리거: ①추정 ▼하향 전환 ②DDR5 현물<계약 롤오버 ③가격>리비전 속도 |
