...
- **승격:** 새로 채택한 관점이 기존 관점군에 붙으면 출처 다양성·numbers·최고점수가 갱신되어 **등급이 자동 재계산**된다. 승격 이력(`E1 7/02 → E2 7/06 → E4 7/09`)은 시각순 재생으로 도출 — 수기 등급 부여가 없다.
...
- 2026-07-12 16:10 · **03 관점과 정보 — 근거 등급(Evidence Grade) 도입**: 채택 관점을 유사도로 묶어 E1 관측 → E2 정황 → E3 확증 → E4 확립 등급 부여, 다른 출처 보강 시 **자동 승격**(시각순 재생 파생 — 스키마 무변경). 60일 무보강=식음(강등 없음). 스트립 정렬 점수순 → **등급순**. **E4라도 숫자 자동 변경 없음.** §3·§8 갱신.
