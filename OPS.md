...
> 2026-08-30 16:43 · **02 Marvell 분기 재무 차트를 첨부 형태로 전환.** 전략 프레임 우측의 8개 분기 수치를 세로 묶음 막대그래프(매출·영업이익)로 렌더링한다. 상단에는 색상 범례와 달력형 `분기별` 표시를 두고, 공통 GAAP·$B 축·수평 격자·우측 눈금·압축 분기 라벨을 제공한다. 실적은 단색, 전망은 저채도·점선으로 구분하며 Q4E 영업이익 `미공시`도 자리표시자로 유지한다. `company.js`의 `.ca-qchart`가 렌더링을 담당하며 620px 이하에서는 축·막대 간격을 축소한다.
