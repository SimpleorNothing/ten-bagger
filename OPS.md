...
| **근거 등급(Evidence Grade) · 관점군 클러스터 · 승격 이력** | 자동(런타임 파생) | 채택할 때마다 재계산 | `insight.js` — R2 `insights.json`을 채택 시각순 재생. **저장 필드 없음**(스키마 무변경) |

**근거 등급 사다리** — 단계(stage)와 무관한 **근거 강도** 축이다.

| 등급 | 조건 | 의미 |
|---|---|---|
| **E1 관측** | 단일 출처 | 아직 근거 아님 |
| **E2 정황** | 독립 2출처 **또는** 단일 고임팩트(N+I+C ≥ 5) | 보강 대기 |
| **E3 확증** | 독립 3출처 **또는** 2출처 + `numbers` 유형 포함 | 판단 투입 가능 |
| **E4 확립** | 3출처 + `numbers` + 최고점수 ≥ 5 | **숫자 검증 대상**(자동 변경은 여전히 없음) |

- **승격:** 새로 채택한 관점이 기존 관점군에 붙으면 출처 다양성·numbers·최고점수가 갱신되어 **등급이 자동 재계산**된다. 승격 이력(`E1 7/02 → E2 7/06 → E4 7/09`)은 채택 시각순 재생으로 도출 — 수기 등급 부여가 없다.
- **클러스터링:** 같은 티커 + 내용 겹침(dice ≥ .18) · 같은 레이어 + 겹침(≥ .28) · 내용만으로 충분히 겹침(≥ .36). 단일연결·오래된 것부터 병합.
- **강등 없음.** 60일 무보강이면 **식음(stale)** 표시만(투명도 감쇠). 틀린 관점은 삭제로 처리.
- **등급은 노출 우선순위를 지배한다** — 01·02·04·05 스트립 정렬 = 점수순이 아니라 **등급순**.

**규율:** 채택돼도, **E4로 승격돼도 숫자는 「반영 대기」**. 파일 변경은 §1 트리거를 통과해야 한다(narrative ≠ numbers).
...
8. **관점 등급이 브리핑에 안 들어온다** — 채택 관점·등급은 **R2 `insights.json` 전용**이라 리포 재페치(§0) 대상이 아니다. 판단에 넣으려면 ① 운영자가 붙여넣거나 ② E3+ 관점을 `signal_log.json`으로 **수기 승격**하거나 ③ R2 → 리포 export(worker cron)를 신설해야 한다. **자동 승격은 설계상 하지 않는다**(봇은 후보 정렬까지).

---

## 갱신 이력

- 2026-07-12 16:20 · **매크로 토픽 축 정규화(`ax`)** — 매 실행 발굴로 같은 축이 다른 이름·id로 들어와 「관련 기사」가 8블록으로 쪠개짐. 파이프라인·렌더를 축 키 기준으로 통일 → **8→5블록**.
...
- 2026-07-12 16:25 · **03 관점과 정보 — 근거 등급(Evidence Grade) 도입**: 채택 관점을 유사도로 묶어 E1 관측 → E2 정황 → E3 확증 → E4 확립 등급 부여, 다른 출처 보강 시 **자동 승격**(채택 시각순 재생 파생 — 스키마 무변경). 60일 무보강=식음(강등 없음). 스트립 정렬 점수순 → **등급순**. **E4라도 숫자 자동 변경 없음.** §3·§8 갱신.
