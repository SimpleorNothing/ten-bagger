...
*마지막 갱신: 2026-05-30 · CASCADES cpu/memory '메모리 인터페이스칩' 초입→가속·cpu 'FC-BGA' 초입→과열 WATCH 정합화(+SIGNAL_LOG 1건) · 03 관통흐름 트리 2노드로 축소(① 강물 / ② 8레이어) — 파생 체인을 강물 value-chain 카드 인라인 드릴다운(.dchain)으로 흡수(cascadeHTML/layerChainHTML, renderCascade 제거) · 메뉴 6탭 유지 · 02 투자 캘린더(`v-cal` 정적 일정 관제) · 01 매크로 매매(signals.json) · 07→05 X-레이에 TARGETS 적정밴드·회전 갭표, xrayHero/portSub HOLDINGS 구동 동기화 · 가장 임박 실무: MU 6/25 (hist ±6.5%, 6/18경 event-iv 갱신) · MRVL 5/27 경과로 제거*
