...
- **메뉴별 업데이트 시점 배지(`.updstamp`):** 각 뷰의 주요 정보 블록 우측 상단에 `update : <KST>` 표기. 정적(수기 큐레이션) 뷰(`updCal`·`updThread`·`updPort`)는 JS 상수 `VIEW_UPDATED`로 일괄 — 콘텐츠 갱신 시 이 상수만 수정. 자동 뷰는 데이터 파일 타임스탬프 연동: `updAlpha`=`alpha.json` asOf, `updMacro`=`signals.json` asOf, 시그널 로그 헤더=`infoAsof()`(최신 `SIGNAL_LOG` at). 포맷은 `fmtUpd()`/`stampText()`.
...
- **X-레이 하단 `#portGapDetail`(개별 종목/ETF 현보유 vs 적정 비중) = `HOLD_DETAIL` 배열 구동.** `w`=계좌 실잔고 비중(레이어 합은 `HOLDINGS`와 일치 — 리밸런싱 시 둘 다 동기화), `tlo~thi`=레이어 밴드(`TARGETS`)를 **레이어 내 향후 상승가능성**(단계·forward γ·상대가치)으로 배분한 운영자 조언(종목 밴드 mid 합 ≈ 레이어 밴드 mid). `dir=trim/add/hold`·`note`=상방 근거. 단일종목 레이어(L4·L7)는 레이어 밴드 직접 적용(L7 증액분은 미보유 800V 신규명 대상). **매매 권유 아님.**
