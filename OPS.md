...
- **메뉴 6탭:** 00 알파맵(`alpha`) / 01 매크로 매매(`macro`) / 02 투자 캘린더(`cal`) / 03 관통흐름(`thread`) / 04 초입지도(`map`) / 05 X-레이(`port`). **`data-v` 키 불변(표시번호만 시프트)** — 섹션 id(`v-{data-v}`)·탭전환 로직의 매칭 키. 01 매크로 매매=`v-macro`(정적 룰북, `signals.json` 신호등). **02 투자 캘린더=`v-cal`(정적 일정 관제: 거시 FOMC·CPI/PCE·한국 금통위·美 중간선거 / 보유·후보 실적 / SpaceX 등 메가이벤트 / 글로벌 워치리스트). 차트 없음 → 탭전환 자동 작동(`map`만 특수처리). 프린트 `order` 배열엔 미포함(macro와 동일 — 도구 페이지는 리포트 출력 제외).** 향후 동적화 시 `calendar.json`(상태·유효기간·트리거) 신설 후보 — signals.json 폴백 패턴 준용, paths-ignore 금지(배포 필요).
- **03 관통 흐름 = `v-thread` 한 섹션 안에 트리(`#threadTree`)로 3개 `.subview` 통합** (구 04 파생체인·05 8레이어 독립탭 흡수): `sub-flow`(강물 S-커브) / `sub-casc`(구 `v-cascade`) / `sub-stack`(구 `v-stack` 스택+24종목 매트릭스). 내부 ID(`threadSvg·cascPick·cascView·stackList·legendStack·mxRoot` 등) **불변** → `renderThread/renderCascade/renderStack` 함수 본문 **무수정**. 외부/로그에서 8레이어로 점프는 **`gotoStack(layerId)`** 사용(구 `.tab[data-v="stack"]` click 대체 — 03 탭→stack 서브뷰 전환 후 `lay-*` 스크롤). 프린트 `order`=`['alpha','macro','cal','thread','map','port']`.
...
*마지막 갱신: 2026-05-30 · 메뉴 6탭(04 파생체인·05 8레이어를 03 관통흐름 한 탭의 트리 서브뷰 sub-flow/sub-casc/sub-stack으로 통합, 내부 ID 불변→렌더 JS 무수정, 점프는 gotoStack) · 02 투자 캘린더(`v-cal` 정적 일정 관제) · 01 매크로 매매(signals.json) · 07→05 X-레이에 TARGETS 적정밴드·회전 갭표, xrayHero/portSub HOLDINGS 구동 동기화 · 가장 임박 실무: MU 6/25 (hist ±6.5%, 6/18경 event-iv 갱신) · MRVL 5/27 경과로 제거*
