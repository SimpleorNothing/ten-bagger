...
> 2026-08-30 16:43 · **02 Marvell 분기 재무 막대그래프 전환.** 전략 프레임 우측의 8개 분기 수치를 2계열 가로 막대그래프(매출·영업이익)로 바꿨다. 공통 GAAP·$B 축, 분기별 FY/CY 라벨, 값 라벨을 유지하고 실적은 실선, 전망은 점선·저채도로 구분한다. Q4E 영업이익 `미공시`도 그래프에서 누락하지 않는다. `company.js`의 `.ca-qchart`가 렌더링을 담당하며 모바일에서는 막대 영역을 축소한다.
