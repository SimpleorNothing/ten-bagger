...
| 정보명 | 자동/수동 | 주기 | 소스 |
|---|---|---|---|
| **AI 수요·공급 로드맵** (04 맨위 · 판정 보드 + 밸류체인 구조도(①~④·돈의 흐름·티어별 손익 스트립·관측 위치) + **이익률 추이 매트릭스(병목의 온도계 — ②랩·③클라우드·④NVDA·④메모리·④통신/전력 연도별 영업이익률 + 행 클릭=요인·구조성·**선행 시그널**[가격·리드타임·캐파·경쟁 진입 4종 — 마진 후행 보정] 판정)** + ①진화 · ②AI 판매자 매트릭스 · ③컴퓨팅 통합(CAPEX 리비전 트랙+4사) · ④Factory 구성요소별 · **④칩 제조사별 Capex(삼성·SK·마이크론·TSMC · 메모리 L3/파운드리 L2·L4 구분)** · ④중국) | 수동 | 분기(실적 시즌 캡처) | `aisd.js` 자가 마운트(#dsAisd · flags.js 패턴 — worker.js `<script defer>` 주입 · v-thread 최상단). 전역 토큰만·`ds-*` 스코프·신규 :root 토큰 0. 수치=컨센서스·공개 실적 방향성, **리비전 트랙·손익·이익률은 캡처 축적 전 예시 표시**. narrative 층 — 숫자 파일 무관. 재판정 트리거: ①추정 ▼하향 ②DDR5 현물<계약 롤오버 ③가격>리비전 속도 · **이익률 서열 역전 = 레이어 회전 신호** |
