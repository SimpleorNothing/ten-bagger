...
- `/api/council-discussions`는 최근 생성 결과를 **최대 20건까지** R2에 누적 보관한다(2026-08-01). 새로고침은 이력을 삭제하지 않으며, 각 항목의 삭제·음성 재생은 그대로 적용한다. `council-sot.js`의 별도 `council.json SoT` 결과·이력 블록은 중복 표시를 막기 위해 로드하지 않는다.
