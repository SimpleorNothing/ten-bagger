...
*마지막 갱신: 2026-06-03 · **가장 임박 실무: AVGO 6/3 AMC(오늘·D-Day)** — 미보유, MRVL 커스텀ASIC 리드스루 게이트(hist ±8%, D-1 event-iv 권장). **MU 6/25 차순위**(hist ±6.5%, 6/18경 event-iv). earnings.json asOf 2026-06-01(AVGO 추가)과 정합. **MU 실적일 6/25 확정**(earnings.json 기준) → 위 v3 플래그의 v-macro 블록7 `MU 6/24` 표기는 정정 필요(미해결 유지). 본문·스키마 무변경 — footer 게이트 포인터만 갱신. (참고: 이 Project에 첨부된 OPS.md 캐시본은 5/24·134줄로 라이브 리포보다 스테일 — 라이브 branch가 SoT.)*
