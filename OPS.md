...
| ↳ **여러 링크 자동 인식·통합**(2026-07-17 신설) | 수동(운영자 입력) | 필요 시 | 유튜브·기사 링크를 **한꺼번에 붙여넣으면** 클라(`recognizeLinks`)가 URL을 파싱→유형 자동 분류(유튜브/기사)→소스별 요약(유튜브=`/api/yt-view` · 기사=신설 `/api/council-read` Claude web_search 본문 읽기)→**하나의 통합 관점으로 합성**(`/api/council-summary` 재사용). 링크 아닌 문장은 메모로 반영. 소스별 진행·한 줄 요약 표시, **실패한 링크는 건너뜀**(개별 처리). 모든 출처 링크는 로그 `refs[]`(신규 필드 · `{label,url}`)에 함께 저장·이력 모달에서 각각 링크로 표시. 신규 CSS·토큰 0(모달 컴포넌트 재사용) |
