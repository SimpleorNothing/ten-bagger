...
| 장중 신호 | `signals.json` | **보조 입력(운영자 수동)** | 운영자/Claude | X (01 매크로 매매 전용) |
...
- **메뉴 7탭:** 00 알파맵(`alpha`) / 01 매크로 매매(`macro`) / 02 관통흐름(`thread`) / 03 파생체인(`cascade`) / 04 8레이어(`stack`) / 05 초입지도(`map`) / 06 X-레이(`port`). **`data-v` 키 불변(표시번호만 시프트)** — 섹션 id(`v-{data-v}`)·탭전환 로직의 매칭 키. 01 매크로 매매=`v-macro`(정적 룰북), `signals.json`(VIX·F&G·사이드카) 읽어 신호등 바인딩 — null이면 `--`/⚪ 폴백.
...
*마지막 갱신: 2026-05-28 · 메뉴 7탭(01 매크로 매매 신설, signals.json) · 가장 임박 실무: MU 6/25 (hist ±6.5%, 6/18경 event-iv 갱신) · MRVL 5/27 경과로 제거*
