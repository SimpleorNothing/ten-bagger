...
12. **patches/ 루트 잔여 .b64 23건(스테일)** — 과거 실패 런·미이관 잔여물. `git apply --check` 실패분이라 push 트리거엔 무해하나, 수동 dispatch 폴백에서 자동 보관 처리됨 → **운영자 1회 dispatch로 일괄 정리 권장**. 재적용 위험이 있던 5건(worker 인사이트 중복·구버전 뉴스 mjs·OPS 부분수정)은 2026-07-14 `patches/applied/`로 이관 완료.

---

## 갱신 이력

- 2026-07-14 22:40 · **OPS 본문 b64 잔류 손상 7곳 수리**(캐프처→캡처 ×2 · 센티멘트→센티먼트 · 썰는다→썩는다 · 펌스링→펄스링 · 안 댈→안 댐 · 굤적→궤적 — 미적용 ops-corrupt-fix 패치가 고치려던 지점) + 재적용 위험 잔여 패치 5건 `applied/` 이관 · §8 이슈 추가.
