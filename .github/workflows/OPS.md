...
- **드래프트 트리아지(A안) 가동(2026-06-03 갱신):** `scripts/derive-drafts.mjs`·`.assetsignore`·**`.github/workflows/update-drafts.yml`(cron 45 21 = 06:45 KST)·`deploy.yml`의 `signal_drafts.json` paths-ignore까지 전부 푸시 완료**(커밋 e2146ba·#114, claude[bot] Action 경유 — MCP가 아니라 in-repo Action이 workflows 스코프 보유). 이전 '미추가·미가동' 서술은 해소. **첫 자동 실행 로그만 Actions 탭에서 1회 확인 권장**(cron 실주기 점등 여부). *시그널 로그 승격은 설계상 영구 수동 — narrative≠numbers, 봇은 후보 정렬까지만.*
- **뉴스 주요내용 사이트 노출 없음** — `news.json`은 리포 전용 원시 피드(검토용), 사이트엔 안 뜸.
- **사이드카 지표 명시 수집 안 됨** — 현재 VIX·S&P·F&G만 수집, 사이드카는 `spDailyPct` 파생.

### 9-6. 자동 데이터층 신뢰성 — 푸시 레이스 (2026-06-03 진단·수정)

**증상:** 6/3 아침 크론에서 `news.json`·`signals.json`은 커밋됐는데 `prices.json`만 미커밋 → 시세 ~24h 스테일. 간헐적(어떤 날은 prices가 이기고 어떤 날은 짐).

**근본 원인 = 푸시 레이스 컨디션.** 5개 데이터 워크플로(`update-prices`·`update-news`·`update-signals` 전부 `37 21 * * *`, `update-drafts` `45 21`, `compute-alpha` 주1회)가 **전부 같은 브랜치로 `git push origin HEAD:claude/wizardly-rubin-SubA1` 하는데 `git pull --rebase`도 retry도 없음.** prices·news·signals가 같은 분에 발사 → 먼저 푸시한 둘이 이기고, 늦은 하나는 non-fast-forward로 거부되어 **조용히 미커밋**(job 실패·알림 없음). `fetch-prices.mjs`는 비치명적 설계(실패해도 last-known 보존)라 fetch는 늘 성공 → **유일 실패점이 push**임이 확정.

**수정안(검증 완료, 반영 대기):** 5개 워크플로의 push 단계를 다음으로 교체.
- `actions/checkout`에 `fetch-depth: 0`(rebase 안정성).
- push 단계 = **`git pull --rebase --autostash origin <branch>` → `git push`** 를 **5회 재시도 루프**(실패 시 `rebase --abort` 후 3~8s 지터 백오프). 진 쪽이 최신 위로 rebase 후 재푸시 → **레이스가 구조적으로 소멸**(cron·fetch 로직·커밋 메시지·paths-ignore 의미 전부 불변).
- YAML·`bash -n`·하드닝 마커 검증 통과한 5개 `.yml` 준비됨.

**반영 경로:** MCP GitHub 연동은 `.github/workflows` 쓰기 스코프가 없어(403 "Resource not accessible by integration") **직접 푸시 불가**. → **claude[bot] Action(워크플로 스코프 보유, §9-5에서 update-drafts.yml을 이 경로로 추가함) 또는 운영자 직접 커밋**으로 5개 .yml 교체. 반영 즉시 §9-1 자동층의 '레이스 미커밋' 리스크 제거.

**오늘자 복구(1회):** 위 수정 반영 전이라도, 스테일한 prices 회복은 Actions 탭 → `Update stock prices` → **Run workflow(workflow_dispatch)** 1회. (단독 실행이라 레이스 없음 → 즉시 커밋.)
...
*2026-06-03 추가 · §9-5 update-drafts 스테일 해소(가동 확인 권장) + §9-6 신설 — 데이터 크론 푸시 레이스 진단(prices 6/3 미커밋의 근본 원인)·수정안(fetch-depth:0 + rebase·5회 재시도). 5개 워크플로 .yml 검증 완료, MCP workflows 스코프 부재로 직접 푸시 불가 → claude[bot] Action/운영자 커밋으로 반영 대기. fetch/cron/커밋메시지/paths-ignore 의미 불변.*
