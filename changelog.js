/* changelog.js — 00~09 메뉴별 업데이트 배지 (자가 마운트).
...
(function(){
  ...
  var MKT_CHANGELOG=[
    {d:'2026-09-12',t:'01 FedWatch 원문시각 정합성 — CME 공식 EOD 직접값 미확보 시 Investing.com 표시값을 2순위로 사용하고 실제 Updated 시각을 보존. 12월 확률 2.6/22.7/47.3/27.4%, 선물 95.910, 원문 Updated Sep 12, 2026 12:35AM EDT 기준으로 fedwatch·시장맥박 동기화'},
    {d:'2026-09-13',t:'01~06 정기 발표 보완 — TreasuryDirect 9월 3Y·10Y·30Y 입찰 결과를 01 카드·시계열·캘린더·rates/cycle/risk·02/04 공유 컨텍스트에 공식값으로 동기화'},
    {d:'2026-09-13',t:'01~06 정기 발표 동기화 — 미 재무부 8월 MTS(월간 적자 $166.8B, 7월 $432.3B, FY26 누적 $1.97T)를 01 카드·시계열·캘린더·시장맥락·사이클·리스크·02/04 공유 signal_log에 공식값으로 동기화'},
    {d:'2026-09-12',t:'01·02·04 Oracle FY27 Q1 공식 실적 동기화 — 매출 $19.345B, OCI $7.4B(+121% YoY), RPO $664B, CAPEX $28.499B, FCF -$5.396B, Q2 Cloud +65~71% 가이던스를 01 카드·RPO 시계열·캘린더·pulse/cycle/risk·02 signal_log·04 버블경보 보조신호에 동시 반영'},
    {d:'2026-09-12',t:'01·02·04 미국 CPI 8월 공식값 동기화 — 헤드라인 +0.4% MoM·+3.4% YoY, 근원 +0.3%·+2.4%; CPI 카드/시계열·캘린더·시장맥락·사이클·리스크·시그널로그·감사이력 동시 반영'},
    ...
    {d:'2026-09-09',t:'01 FedWatch 원문시각 정합성 — CME 공식 EOD 직접값 미확보 시 Investing.com 표시값을 2순위로 사용하고 실제 Updated 시각을 보존. 12월 확률 13.6/41.3/36.8/8.2%, 선물 96.060, 원문 Updated Sep 09, 2026 04:05AM EDT 기준으로 fedwatch·시장맥박 동기화'},
    {d:'2026-09-09',t:'01~06 정기 발표 동기화 — 중국 해관총서 8월 무역(수출 +25.0%·수입 +28.2% YoY, 흑자 $119.09B)을 01 카드·시계열·캘린더·시장맥락·사이클·리스크·02/04 공유 signal_log에 공식값으로 동기화'},
    ...
    {d:'2026-09-07',t:'01·02·04 정기 발표 동기화 — SIA/WSTS 7월 글로벌 반도체 판매 $146.8B(+6.4% MoM, +135.1% YoY)·6월 수정 $137.9B를 카드·시계열·캘린더·시장맥락·사이클·리스크 근거·02 시그널로그에 교차 반영'},
    {d:'2026-09-05',t:'공통 초기 로딩 복구 — 07 투자일지 탭 추가 후 00 시장 지도의 메뉴 번호 감시기가 자기 변경을 다시 감지하던 순환을 차단. 번호가 실제로 달라질 때만 DOM을 갱신하도록 두 모듈을 멱등화하고 재발 방지 회귀 검사를 추가'},
    {d:'2026-09-05',t:'05 토요일 원장 정합성 — 자산현황_260905.xlsx를 권위값으로 동기화하고 신규 AVGO·CRDO/DRAM3배 청산을 holdings·held·news 필터에 반영. 01 FedWatch는 Investing.com 05:55 KST 2순위값(15.5/41.7/34.5/8.3%, 선물 96.065)으로 갱신하고 보유뉴스·시장맥박 6축을 ChatGPT 자체 추론으로 재생성'},
    {d:'2026-09-05',t:'01 정기 발표 동기화 — BLS 8월 고용 NFP +162K·실업률 4.1%·임금 +3.1% YoY와 6·7월 +55K 상향수정, 한국 8월 CPI +3.1% YoY를 카드·캘린더·시장맥락·CPI 시계열에 반영'},
    {d:'2026-09-04',t:'공통 시장 티커와 01 WTI 카드가 과거 일봉보다 근월물 실시간 시세를 우선하도록 수정해 급등 구간의 오래된 유가 표시를 방지'},
    {d:'2026-09-02',t:'01 정기 발표 점검 — BLS 7월 JOLTS 구인 727.1만·6월 718.2만으로 하향 수정, 중국 8월 제조업 PMI 49.8을 공식 원문 기준으로 캘린더·시장맥락에 동기화'},
    {d:'2026-08-31',t:'01 다가오는 일정에 Broadcom FY26 Q3 실적 발표를 회사 공식 IR 기준 9월 3일 06:00 KST로 추가'},
    {d:'2026-08-31',t:'01 다가오는 일정에 미국 JOLTS 구인·이직 보고서를 추가하고, BLS 직접 접근 차단 시 공식 원문 보조 읽기 경로까지 검증해 매월 한국시간으로 자동 동기화'},
    {d:'2026-08-30',t:'01 시장 맥박 축별 기사 제목을 한글로 통일하고, 자동 갱신 규칙·반복 영문 제목 보정 매핑을 적용'},
    {d:'2026-08-30',t:'01 시장 맥박의 축별 근거 기사를 원문 날짜 최신순으로 정렬 — 최신 3건을 위에서부터 표시하고 날짜가 없거나 잘못된 항목은 아래로 배치'},
    {d:'2026-08-30',t:'01 종목 뉴스에서 기사 없는 글로벌HBM반도체(442580)·삼성·SK 채권혼합(0162Z0) 빈 카드를 제거하고, 주간 보유자산 동기화 후에도 다시 생성되지 않도록 제외 규칙 적용'},
    {d:'2026-08-29',t:'05 투자매력도 V4.1 — 성장성이 유지되는 종목의 일시 급락을 별도 포착: 최근 급락·잔존 조정폭과 FY+1/근접 EPS 리비전, 목표가, 컨센서스, γ를 함께 검증해 펀더멘털 훼손이 없을 때만 급락기회 가점과 가격-EPS 괴리 리셋을 반영'},
    {d:'2026-08-28',t:'01 EIA 8월 26일 주간 원유재고와 8월 STEO를 재동기화 — 단기 재고 완충과 2026년 말까지의 중기 재고 타이트 전망을 분리해 유가/에너지 시장 맥락에 반영'},
    {d:'2026-08-28',t:'01 정기 발표 동기화 — 한국은행 기준금리 3.00%(+25bp), NVIDIA FY27 Q2 매출 $96.2B·데이터센터 $89.0B, Marvell FY27 Q2 매출 $2.739B·데이터센터 +46% YoY와 상향 가이던스를 캘린더·시장 맥락·02 기업뉴스·실적 이벤트 상태에 반영'},
    {d:'2026-08-27',t:'01 BEA 2Q26 GDP 2차 추정(+1.5% SAAR)과 7월 PCE(헤드라인 +3.7% YoY·근원 +3.3%) 공식 발표를 캘린더와 금리/Fed 시장 맥락에 동기화'},
    {d:'2026-08-26',t:'01 골드만삭스 2Q26 기관 수급을 반영 — 반도체의 빠른돈→느린돈 손바뀜, AI 인프라·금융 공통매수, MRVL shared favorites 이탈을 시점 지연과 현재 보유 집중도까지 포함해 시장 맥락으로 기록'},
    {d:'2026-08-23',t:'01 재무부 장기채 바이백 확대를 QE와 구분하고, 단기채·ON RRP·은행 준비금 경로 및 11월 4일 분기발행계획 관전 포인트를 시장 맥락과 일정에 반영'},
    {d:'2026-08-17',t:'01 상단 투자 명언 배너 — F&G가 공포일수록 파란색, 탐욕일수록 빨간색으로 진해지는 그라데이션 적용'},
    {d:'2026-08-16',t:'05 투자매력도 V4 적용 — Nasdaq-100 순위 백분위, 중복 감점 제거, 실제 밸류·가격-EPS 괴리, 외생 데이터와 내부 판단 분리 및 일일 점수 스냅샷'},
    {d:'2026-08-16',t:'05 투자매력도의 EPS 리비전 점수를 Nasdaq-100 당일 P10~P90 실측 기준으로 재산출하고 화면에 기준일·유효표본을 표시'},
    ...
    {d:'2026-08-08',t:'01 미국 비농업고용(NFP) 7월 -23K·5월 +63K·6월 +20K 공식 수정치를 반영하고, BLS 공식 백필을 FRED 지연값보다 우선하도록 정합성 경로 수정'},
    {d:'2026-08-05',t:'03 전문가 원탁의 이력 제목을 「토론 이력」으로 단순화하고 수동 새로고침 버튼 제거'},
    {d:'2026-08-03',t:'02 인사이트의 채택·검증 매크로 관점을 01 시장 맥박의 해당 축에도 자동 연결해, FOMC 결과가 금리/Fed 카드에 계속 반영되도록 보완'},
