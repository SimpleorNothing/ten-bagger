...
#portfolioHistoryDownload{margin:22px 0 24px;padding:0;scroll-margin-bottom:calc(var(--am-ticker-h) + 96px)}.ph-box{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:18px 20px;border:1px solid var(--line);border-radius:14px;background:var(--panel)}.ph-title{font-weight:800;font-size:16px}.ph-sub{margin-top:3px;color:var(--dim);font-size:12px}.ph-open{flex:0 0 auto;border:1px solid var(--line2);background:var(--dawn);color:var(--onacc);font:700 13px var(--sans);padding:9px 13px;border-radius:9px;cursor:pointer}.ph-bg{position:fixed;inset:0;z-index:130;background:rgba(4,10,15,.48);backdrop-filter:blur(3px);display:none}.ph-bg.on{display:block}.ph-modal{position:fixed;z-index:131;left:50%;top:50%;transform:translate(-50%,-50%);width:min(820px,94vw);max-height:84vh;overflow:auto;background:var(--ink2);border:1px solid var(--line2);border-radius:16px;box-shadow:0 24px 70px rgba(0,0,0,.28);display:none;padding:20px}.ph-modal.on{display:block}.ph-head{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-bottom:14px}.ph-head h3{font-size:20px}.ph-note{font-size:12px;color:var(--dim);margin-top:3px}.ph-close{width:32px;height:32px;border:1px solid var(--line);border-radius:8px;background:var(--panel);cursor:pointer;color:var(--txt)}.ph-status{font-size:12px;color:var(--dim);padding:8px 0}.ph-controls{display:flex;align-items:center;gap:8px;flex-wrap:wrap;border-top:1px solid var(--line);border-bottom:1px solid var(--line);padding:12px 0}.ph-controls label{font-size:12px;font-weight:700}.ph-select,.ph-actions a{border:1px solid var(--line);background:var(--panel);color:var(--txt);padding:7px 9px;border-radius:7px;text-decoration:none;font:600 12px var(--sans)}.ph-actions{display:flex;gap:6px;flex-wrap:wrap;margin-left:auto}.ph-detail{margin-top:14px;border-top:2px solid var(--line2);padding-top:12px}.ph-detail h4{font-size:15px;margin:4px 0 8px}.ph-account{margin:12px 0 5px;font-weight:800;font-size:12px}.ph-tablewrap{overflow:auto;border:1px solid var(--line);border-radius:9px}.ph-table{width:100%;border-collapse:collapse;min-width:640px;font-size:11px}.ph-table th,.ph-table td{padding:7px 8px;border-bottom:1px solid var(--line);text-align:right;white-space:nowrap}.ph-table th{background:var(--panel2);color:var(--dim);font-weight:700}.ph-table th:first-child,.ph-table td:first-child{text-align:left}.ph-code{display:block;color:var(--faint);font-size:10px;margin-top:2px}.ph-empty{padding:22px 4px;color:var(--dim);text-align:center;font-size:13px}@media(max-width:700px){.ph-box{align-items:flex-start}.ph-modal{padding:16px}.ph-sub{max-width:230px}.ph-actions{margin-left:0;width:100%}}';document.head.appendChild(st);
  }
  ...
  function renderList(data){
    ...
    var options=dates.map(function(x){return '<option value="'+esc(x.date)+'">'+esc(x.date)+' · '+nf.format(Number(x.holdingCount||0))+'종목 · '+money(x.holdingValueKrw)+'</option>';}).join('');
    listHost.innerHTML='<div class="ph-controls"><label for="phDate">기준일</label><select id="phDate" class="ph-select">'+options+'</select><div class="ph-actions"><a href="'+API+'.csv" download>전체 기간 Excel</a><a id="phDateCsv" href="'+API+'/'+encodeURIComponent(dates[0].date)+'.csv" download>선택일 CSV</a><a id="phDateJson" href="'+API+'/'+encodeURIComponent(dates[0].date)+'.json" download>선택일 JSON</a></div></div>';
    var select=listHost.querySelector('#phDate');var csv=listHost.querySelector('#phDateCsv');var json=listHost.querySelector('#phDateJson');
    function selectDate(){var date=select.value;csv.href=API+'/'+encodeURIComponent(date)+'.csv';json.href=API+'/'+encodeURIComponent(date)+'.json';showDetail(date);}
    select.addEventListener('change',selectDate);selectDate();
  }
  ...
  function mount(){
    ...
    accountView.style.paddingBottom='calc(var(--am-ticker-h) + 96px)';
    injectStyle();
