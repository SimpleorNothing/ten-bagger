...
async function historyCsvAll(env){const dates=await listHistory(env),header=['기준일','계좌','데이터원','시장','종목코드','종목명','수량','매입가','현재가','평가금액(원)','평가손익(원)','수익률(%)'],body=[];for(const item of dates.slice().reverse()){const stored=await getStoredHistory(env,item.date);if(!stored)continue;for(const row of flattenPortfolioHoldings(stored.snapshot||{}))body.push([stored.snapshotDate,row.account,row.dataSource,row.market,row.code,row.name,row.quantity,row.purchasePrice,row.currentPrice,row.evaluationAmountKrw,row.profitLossKrw,row.returnPct].map(csvCell).join(','));}return '\uFEFF'+[header.map(csvCell).join(','),...body].join('\r\n');}
