...
<div class="ds-topcharts" aria-label="컴퓨팅 판매자 핵심 지표 차트">
  <section class="ds-topchart wide">
    ...
    <div class="ds-topfn">좌측 축=수주잔고(최대 2.00 · 0.50 간격) · 우측 축=클라우드 서비스 매출·CAPEX(최대 300B · 100B 간격) · 막대=수주잔고·매출·CAPEX · 매출 괄호는 수주잔고 대비, CAPEX 괄호는 매출 대비 · Azure 매출은 공시 성장률 기반 환산, AWS 수주잔고는 1Q26 기준(신규 OpenAI $100B 제외)</div>
  </section>
</div>
...
function mountQuarterlyCloudChart(root){
 ...
 function fmtL(v){return (v/1000).toFixed(2)} function fmtR(v){return v+'B'}
 function col(name){return getComputedStyle(root).getPropertyValue(name).trim()||'#496176'}
 function draw(){var r=box.getBoundingClientRect(),dpr=Math.min(window.devicePixelRatio||1,2),w=Math.max(290,Math.round(r.width)),h=Math.max(220,Math.round(r.height));canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);
  ...
  ctx.font='11px '+getComputedStyle(root).getPropertyValue('--mono');ctx.textBaseline='middle';ctx.strokeStyle=col('--line');ctx.fillStyle=col('--faint');ctx.lineWidth=1;[0,500,1000,1500,2000].forEach(function(v){var yy=yL(v);ctx.beginPath();ctx.moveTo(pad.l,yy+.5);ctx.lineTo(w-pad.r,yy+.5);ctx.stroke();ctx.textAlign='right';ctx.fillText(fmtL(v),pad.l-7,yy)});[0,100,200,300].forEach(function(v){ctx.textAlign='left';ctx.fillStyle=col('--dim');ctx.fillText(fmtR(v),w-pad.r+7,yR(v))});
  var bw=Math.max(7,Math.min(21,pw/d.years.length*.24));d.backlog.forEach(function(v,i){var xx=x(i)-bw*1.5-2,yy=yL(v);ctx.fillStyle=col('--dawn');ctx.fillRect(xx,yy,bw,yL(0)-yy);ctx.fillStyle=col('--txt');ctx.textAlign='center';ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText(fmtL(v),x(i)-bw-2,Math.max(12,yy-8))});
  d.revenue.forEach(function(v,i){var xx=x(i)-bw/2,yy=yR(v);ctx.fillStyle=col('--st-hot');ctx.fillRect(xx,yy,bw,yR(0)-yy);ctx.fillStyle=col('--st-hot');ctx.textAlign='center';ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');var ly=Math.max(12,yy-12);ctx.fillText(fmtR(v),x(i),ly);ctx.font='10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText('('+Math.round(v/d.backlog[i]*100)+')',x(i),ly+11)});
  d.capex.forEach(function(v,i){var xx=x(i)+bw/2+2,yy=yR(v);ctx.fillStyle=col('--st-mature');ctx.fillRect(xx,yy,bw,yR(0)-yy);ctx.fillStyle=col('--st-mature');ctx.textAlign='center';ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');var ly=Math.max(12,yy-12);ctx.fillText(fmtR(v),x(i)+bw+2,ly);ctx.font='10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText('('+Math.round(v/d.revenue[i]*100)+')',x(i)+bw+2,ly+11)});
 ...
 function show(e){var r=canvas.getBoundingClientRect(),px=e.clientX-r.left;active=Math.max(0,Math.min(d.years.length-1,Math.round((px-47)/(r.width-63)*d.years.length-.5)));var i=active;tip.innerHTML='<b>'+d.years[i]+'</b><br>수주잔고 · '+fmtL(d.backlog[i])+'<br>매출 · '+fmtR(d.revenue[i])+'<br>('+Math.round(d.revenue[i]/d.backlog[i]*100)+')<br>CAPEX · '+fmtR(d.capex[i])+'<br>('+Math.round(d.capex[i]/d.revenue[i]*100)+')';tip.style.display='block';tip.style.left=Math.min(r.width-164,Math.max(4,px+10))+'px';tip.style.top='7px';draw()}
