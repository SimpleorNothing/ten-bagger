...
<div class="ds-topcharts" aria-label="컴퓨팅 판매자 핵심 지표 차트">
  <section class="ds-topchart wide">
    ...
    <div class="ds-topfn">좌측 축=수주잔고(최대 2.00T · 0.50T 간격) · 우측 축=클라우드 서비스 매출·CAPEX(최대 0.30T · 0.10T 간격) · 막대=수주잔고·매출·CAPEX · 매출 괄호는 수주잔고 대비, CAPEX 괄호는 매출 대비 · Azure 매출은 공시 성장률 기반 환산, AWS 수주잔고는 1Q26 기준(신규 OpenAI $100B 제외)</div>
  </section>
</div>
...
function mountQuarterlyCloudChart(root){
 ...
 function fmt(v){return (v/1000).toFixed(2)+'T'}
 function col(name){return getComputedStyle(root).getPropertyValue(name).trim()||'#496176'}
 ...
 function show(e){var r=canvas.getBoundingClientRect(),px=e.clientX-r.left;active=Math.max(0,Math.min(d.years.length-1,Math.round((px-47)/(r.width-63)*d.years.length-.5)));var i=active;tip.innerHTML='<b>'+d.years[i]+'</b><br>수주잔고 · '+fmt(d.backlog[i])+'<br>매출 · '+fmt(d.revenue[i])+'<br>('+Math.round(d.revenue[i]/d.backlog[i]*100)+')<br>CAPEX · '+fmt(d.capex[i])+'<br>('+Math.round(d.capex[i]/d.revenue[i]*100)+')';tip.style.display='block';tip.style.left=Math.min(r.width-164,Math.max(4,px+10))+'px';tip.style.top='7px';draw()}
