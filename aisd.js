...
<div class="ds-topcharts" aria-label="컴퓨팅 판매자 핵심 지표 차트">
  <section class="ds-topchart wide">
    ...
    <div class="ds-topfn">좌측 축=수주잔고 · 우측 축=클라우드 서비스 매출·CAPEX · 막대=수주잔고·CAPEX, 라인=매출 · 매출 괄호는 수주잔고 대비, CAPEX 괄호는 매출 대비 · Azure 매출은 공시 성장률 기반 환산, AWS 수주잔고는 1Q26 기준(신규 OpenAI $100B 제외)</div>
  </section>
</div>
...
function mountQuarterlyCloudChart(root){
 ...
 function draw(){var r=box.getBoundingClientRect(),dpr=Math.min(window.devicePixelRatio||1,2),w=Math.max(290,Math.round(r.width)),h=Math.max(220,Math.round(r.height));canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);
  var pad={l:47,r:47,t:24,b:39},pw=w-pad.l-pad.r,ph=h-pad.t-pad.b,leftMax=1800,rightMax=180,x=function(i){return pad.l+pw*(i+.5)/d.years.length},yL=function(v){return pad.t+ph*(1-v/leftMax)},yR=function(v){return pad.t+ph*(1-v/rightMax)};
  ctx.font='11px '+getComputedStyle(root).getPropertyValue('--mono');ctx.textBaseline='middle';ctx.strokeStyle=col('--line');ctx.fillStyle=col('--faint');ctx.lineWidth=1;[0,600,1200,1800].forEach(function(v){var yy=yL(v);ctx.beginPath();ctx.moveTo(pad.l,yy+.5);ctx.lineTo(w-pad.r,yy+.5);ctx.stroke();ctx.textAlign='right';ctx.fillText(fmt(v),pad.l-7,yy)});[0,60,120,180].forEach(function(v){ctx.textAlign='left';ctx.fillStyle=col('--dim');ctx.fillText(fmt(v),w-pad.r+7,yR(v))});
  var bw=Math.max(8,Math.min(28,pw/d.years.length*.33));d.backlog.forEach(function(v,i){var xx=x(i)-bw-2,yy=yL(v);ctx.fillStyle=col('--dawn');ctx.fillRect(xx,yy,bw,yL(0)-yy);ctx.fillStyle=col('--txt');ctx.textAlign='center';ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText(fmt(v),x(i)-bw/2-2,Math.max(12,yy-8))});
  d.capex.forEach(function(v,i){var xx=x(i)+2,yy=yR(v);ctx.fillStyle=col('--st-mature');ctx.fillRect(xx,yy,bw,yR(0)-yy);ctx.fillStyle=col('--st-mature');ctx.textAlign='center';ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText(fmt(v)+' ('+Math.round(v/d.revenue[i]*100)+'%)',x(i)+bw/2+2,Math.max(12,yy-8))});
  ctx.strokeStyle=col('--st-hot');ctx.fillStyle=col('--st-hot');ctx.lineWidth=2;ctx.beginPath();d.revenue.forEach(function(v,i){var xx=x(i),yy=yR(v);i?ctx.lineTo(xx,yy):ctx.moveTo(xx,yy)});ctx.stroke();d.revenue.forEach(function(v,i){var xx=x(i),yy=yR(v);ctx.beginPath();ctx.arc(xx,yy,3.5,0,Math.PI*2);ctx.fill();ctx.fillStyle=col('--st-hot');ctx.textAlign='center';ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText(fmt(v)+' ('+Math.round(v/d.backlog[i]*100)+'%)',xx,Math.min(h-pad.b-9,yy+17))});
