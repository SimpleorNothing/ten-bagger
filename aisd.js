...
function mountQuarterlyCloudChart(root){
 ...
 function show(e){var r=canvas.getBoundingClientRect(),px=e.clientX-r.left;active=Math.max(0,Math.min(d.years.length-1,Math.round((px-47)/(r.width-63)*d.years.length-.5)));var i=active;tip.innerHTML='<b>'+d.years[i]+'</b><br>수주잔고 · '+fmtLValue(d.backlog[i])+'<br>매출 · '+fmtRValue(d.revenue[i])+'<br>('+Math.round(d.revenue[i]/d.backlog[i]*100)+')<br>CAPEX · '+fmtRValue(d.capex[i])+'<br>('+Math.round(d.capex[i]/d.revenue[i]*100)+')';tip.style.display='block';tip.style.left=Math.min(r.width-164,Math.max(4,px+10))+'px';tip.style.top='7px';draw()}
