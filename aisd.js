...
<div class="ds-card">
  <div class="ds-capex-toggles" role="group" aria-label="차트 시리즈 표시">
    <button type="button" data-series="capex" aria-pressed="true">CAPEX</button><button type="button" data-series="revenue" aria-pressed="true">매출</button><button type="button" data-series="fcf" aria-pressed="true">FCF</button><button type="button" data-series="op" aria-pressed="true">영업이익</button>
  </div>
...
function mountCapexChart(root){
 ...
 var data={capex:{label:'CAPEX',values:[150,230,460,700,1000,1200],display:['$150B','$230B','$460B','~$700B','>$1,000B','~$1,200B'],yoy:[null,53,100,52,43,20],color:'--dawn',bar:true},revenue:{label:'매출',values:[1250,1420,1620,1850,2100,2400],display:['$1.25T','$1.42T','~$1.62T','~$1.85T','~$2.1T','~$2.4T'],yoy:[null,14,14,14,14,14],color:'--st-dawn'},fcf:{label:'FCF',values:[210,230,170,0,null,null],color:'--st-hot'},op:{label:'영업이익',values:[270,370,440,500,560,null],color:'--st-mature'}};
 ...
 function draw(){
  ...
  ['revenue','fcf','op'].forEach(function(key){if(!visible[key])return;var d=data[key];ctx.strokeStyle=col(d.color);ctx.fillStyle=col(d.color);ctx.lineWidth=2;for(var seg=0;seg<2;seg++){ctx.setLineDash(seg?[5,4]:[]);ctx.beginPath();var began=false;d.values.forEach(function(v,i){if(v==null||(seg===0&&i>2)||(seg===1&&i<2))return;var xx=x(i),yy=y(v);began?ctx.lineTo(xx,yy):ctx.moveTo(xx,yy);began=true});ctx.stroke()}ctx.setLineDash([]);d.values.forEach(function(v,i){if(v==null)return;var xx=x(i),yy=y(v);ctx.beginPath();ctx.arc(xx,yy,4,0,Math.PI*2);ctx.fillStyle=i<3?col(d.color):col('--panel2');ctx.fill();ctx.strokeStyle=col(d.color);ctx.lineWidth=1.5;ctx.stroke();hit.push({i:i,x:xx,y:yy,key:key});if(key==='revenue'){ctx.textAlign='center';ctx.fillStyle=col(d.color);ctx.font='12px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText(d.display[i],xx,Math.max(18,yy-11));if(d.yoy[i]!=null){ctx.fillStyle=col('--dim');ctx.fillText('+'+d.yoy[i]+'%',xx,Math.max(8,yy-27))}}})});
  if(active>=0){ctx.fillStyle='rgba(73,97,118,.07)';ctx.fillRect(x(active)-pw/years.length/2,pad.t,pw/years.length,ph)}
 }
 function show(e){var r=canvas.getBoundingClientRect(),px=e.clientX-r.left,idx=Math.max(0,Math.min(5,Math.round((px-48)/(r.width-64)*6-.5)));active=idx;var lines=['<b>'+years[idx]+'</b>'];Object.keys(data).forEach(function(k){var v=data[k].values[idx];if(visible[k]&&v!=null)lines.push(data[k].label+' · '+fmt(v))});tip.innerHTML=lines.join('<br>');tip.style.display='block';tip.style.left=Math.min(r.width-170,Math.max(4,px+12))+'px';tip.style.top='8px';draw()}
