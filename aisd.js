...
#dsAisd .ds-capex-chart{position:relative;height:420px;min-height:320px;border-bottom:1px solid var(--line2)}
#dsAisd .ds-capex-chart canvas{display:block;width:100%;height:100%;touch-action:manipulation}
#dsAisd .ds-capex-toggles{display:flex;gap:7px;flex-wrap:wrap;margin:0 0 10px}
#dsAisd .ds-capex-toggles button{font-family:var(--mono);font-size:12px;font-weight:700;color:var(--txt);background:var(--panel2);border:1px solid var(--line2);border-radius:20px;padding:5px 10px;cursor:pointer}
...
#dsAisd .ds-capex-chart{height:340px;min-height:300px}
...
<div class="ds-topcharts" aria-label="컴퓨팅 판매자 핵심 지표 차트">
  <section class="ds-topchart wide">
    ...
    <div class="ds-l2">AWS·Microsoft Cloud·Google Cloud 매출, 3사 공개 수주잔고·CAPEX 합산 — Meta 제외</div>
  ...
  <section class="ds-topchart wide">
    ...
    <div class="ds-topfn">단위 $B · RPO=연말 잔액 · 매출=AWS+Microsoft Cloud+Google Cloud · CAPEX=Amazon+Microsoft+Alphabet 전사 기준 · 2026E 이후 전망치는 Base Case이며 실적과 구분해 E 표기로 표시 · 꺾은선 그래프</div>
  </section>
</div>
...
<div class="ds-card">
  ...
  <div class="ds-capex-chart" data-capex-chart="detail"><canvas tabindex="0" role="img" aria-label="2023년부터 2028년까지 컴퓨팅 판매자 4사의 CAPEX, 매출, FCF, 영업이익 차트"></canvas><div class="ds-capex-tip" role="status" aria-live="polite"></div></div>
...
function mountCapexChart(root,which){
 var box=root.querySelector(which||'.ds-capex-chart'),canvas=box&&box.querySelector('canvas'),tip=box&&box.querySelector('.ds-capex-tip');if(!canvas)return;
 var ctx=canvas.getContext('2d'),years=['2023','2024','2025','2026E','2027E','2028E'],visible={capex:true,revenue:true,fcf:true,op:true},hit=[],active=-1,labels=[];
 ...
 function queueLabel(i,text,xx,yy,color,bold,anchorY){labels[i].push({text:text,x:xx,y:yy,color:color,bold:bold,anchorY:anchorY})}
 function drawLabels(top,bottom){
  labels.forEach(function(items){if(!items.length)return;items.sort(function(a,b){return a.y-b.y});var gap=22;
   items.forEach(function(it,j){if(j&&it.y<items[j-1].y+gap)it.y=items[j-1].y+gap});var over=items[items.length-1].y-bottom;if(over>0)items.forEach(function(it){it.y-=over});
   for(var j=items.length-2;j>=0;j--)if(items[j].y>items[j+1].y-gap)items[j].y=items[j+1].y-gap;var under=top-items[0].y;if(under>0)items.forEach(function(it){it.y+=under});
   items.forEach(function(it){if(Math.abs(it.y-it.anchorY)>13){ctx.strokeStyle=col('--line2');ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(it.x,it.anchorY);ctx.lineTo(it.x,it.y);ctx.stroke()}label(it.text,it.x,it.y,it.color,it.bold)})
  })
 }
 function draw(){
  var r=box.getBoundingClientRect(),dpr=Math.min(window.devicePixelRatio||1,2),w=Math.max(280,Math.round(r.width)),h=Math.max(260,Math.round(r.height));canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);hit=[];labels=years.map(function(){return[]});
  ...
  if(visible.capex){var d=data.capex,bw=Math.min(54,pw/years.length*.55);d.values.forEach(function(v,i){var xx=x(i)-bw/2,yy=y(v);ctx.fillStyle=i<3?col(d.color):col('--panel2');ctx.strokeStyle=col(d.color);ctx.lineWidth=1.5;ctx.fillRect(xx,yy,bw,y(0)-yy);if(i>=3){ctx.setLineDash(i===5?[5,4]:[]);ctx.strokeRect(xx+.75,yy+.75,bw-1.5,y(0)-yy-1);ctx.setLineDash([])}hit.push({i:i,x:x(i),y:yy,key:'capex'});queueLabel(i,d.display[i],x(i),Math.max(22,yy-12),col('--txt'),true,yy);if(d.yoy[i]!=null)queueLabel(i,'+'+d.yoy[i]+'%',x(i),Math.max(12,yy-34),col('--dim'),false,yy)})}
  ['revenue','fcf','op'].forEach(function(key){if(!visible[key])return;var d=data[key];ctx.strokeStyle=col(d.color);ctx.fillStyle=col(d.color);ctx.lineWidth=2;for(var seg=0;seg<2;seg++){ctx.setLineDash(seg?[5,4]:[]);ctx.beginPath();var began=false;d.values.forEach(function(v,i){if(v==null||(seg===0&&i>2)||(seg===1&&i<2))return;var xx=x(i),yy=y(v);began?ctx.lineTo(xx,yy):ctx.moveTo(xx,yy);began=true});ctx.stroke()}ctx.setLineDash([]);d.values.forEach(function(v,i){if(v==null)return;var xx=x(i),yy=y(v);ctx.beginPath();ctx.arc(xx,yy,4,0,Math.PI*2);ctx.fillStyle=i<3?col(d.color):col('--panel2');ctx.fill();ctx.strokeStyle=col(d.color);ctx.lineWidth=1.5;ctx.stroke();hit.push({i:i,x:xx,y:yy,key:key});if(key==='revenue'){queueLabel(i,d.display[i],xx,Math.max(22,yy-12),col(d.color),true,yy);if(d.yoy[i]!=null)queueLabel(i,'+'+d.yoy[i]+'%',xx,Math.max(12,yy-34),col('--dim'),false,yy)}else{var margin=Math.round(v/data.revenue.values[i]*100),ly=key==='op'?yy-18:Math.min(h-pad.b-12,yy+20);queueLabel(i,d.display[i]+' ('+margin+'%)',xx,ly,col(d.color),true,yy)}})});
  drawLabels(pad.t-10,h-pad.b-10);
 ...
 canvas.addEventListener('pointermove',show);canvas.addEventListener('pointerdown',show);canvas.addEventListener('keydown',function(e){if(e.key==='ArrowRight'||e.key==='ArrowLeft'){e.preventDefault();active=Math.max(0,Math.min(5,(active<0?0:active)+(e.key==='ArrowRight'?1:-1)));var r=canvas.getBoundingClientRect();show({clientX:r.left+48+(r.width-64)*(active+.5)/6})}});canvas.addEventListener('pointerleave',function(e){if(e.pointerType!=='touch'){active=-1;tip.style.display='none';draw()}});
...
function mountQuarterlyCloudChart(root){
 ...
 // 수주잔고=Microsoft 상업 RPO·Google Cloud revenue backlog·AWS RPO의 실제 분기 공개액. 3사 모두 값이 있는 분기만 합산한다.
 // CAPEX=Amazon·Alphabet 회사 전체 CAPEX와 Microsoft 금융리스 포함 CAPEX. 영업이익률은 공개 부문 지표의 매출 가중 참고치. 단위: $B.
 var d={years:['24.1Q','24.2Q','24.3Q','24.4Q','25.1Q','25.2Q','25.3Q','25.4Q','26.1Q','26.2Q'],backlog:[465.2,504.4,509.8,568.2,596,669,747,1112,1459,1688],revenue:[69.7,73.4,77.8,81.7,84,91.1,97.3,104.8,112.1,126.3],capex:[40.9,49.8,55.7,64.7,63.6,78.7,93.1,105.5,110.8,140.1],opMargin:[35,34,36,35,37,35,36,40,40,39],companies:[{name:'Microsoft',color:'#315b78',share:{backlog:[0.505159,0.533307,0.508042,0.524463,0.528523,0.550075,0.524766,0.56205,0.429746,0.401659],revenue:[0.503587,0.501362,0.5,0.500612,0.504762,0.512623,0.504625,0.491412,0.486173,0.469517],capex:[0.342298,0.381526,0.359066,0.349304,0.336478,0.307497,0.374866,0.35545,0.287906,0.292648]}},{name:'Amazon',color:'#5f86a0',share:{backlog:[0.338994,0.310468,0.321695,0.31151,0.317114,0.29148,0.267738,0.219424,0.249486,0.293839],revenue:[0.35868,0.358311,0.35347,0.352509,0.34881,0.33809,0.339157,0.339695,0.335415,0.334125],capex:[0.364303,0.353414,0.405745,0.429675,0.393082,0.407878,0.367347,0.383886,0.389892,0.386867]}},{name:'Alphabet',color:'#9bb5c5',share:{backlog:[0.155847,0.156225,0.170263,0.164027,0.154362,0.158445,0.207497,0.218525,0.320768,0.304502],revenue:[0.137733,0.140327,0.14653,0.146879,0.146429,0.149286,0.156218,0.168893,0.178412,0.196358],capex:[0.293399,0.26506,0.235189,0.22102,0.27044,0.284625,0.257787,0.260664,0.322202,0.320485]}}]},ctx=canvas.getContext('2d'),active=-1;
 function fmtLAxis(v){return Math.round(v)+'B'} function fmtLValue(v){return String(Math.round(v))} function fmtRAxis(v){return Math.round(v)+'B'} function fmtRValue(v){return String(Math.round(v))}
 function col(name){return getComputedStyle(root).getPropertyValue(name).trim()||'#496176'}
 function draw(){var r=box.getBoundingClientRect(),dpr=Math.min(window.devicePixelRatio||1,2),w=Math.max(290,Math.round(r.width)),h=Math.max(220,Math.round(r.height));canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);
  var pad={l:47,r:47,t:24,b:39},pw=w-pad.l-pad.r,ph=h-pad.t-pad.b,leftMax=2000,rightMax=300,x=function(i){return pad.l+pw*(i+.5)/d.years.length},yL=function(v){return pad.t+ph*(1-v/leftMax)},yR=function(v){return pad.t+ph*(1-v/rightMax)};
  ctx.font='11px '+getComputedStyle(root).getPropertyValue('--mono');ctx.textBaseline='middle';ctx.strokeStyle=col('--line');ctx.fillStyle=col('--faint');ctx.lineWidth=1;[0,500,1000,1500,2000].forEach(function(v){var yy=yL(v);ctx.beginPath();ctx.moveTo(pad.l,yy+.5);ctx.lineTo(w-pad.r,yy+.5);ctx.stroke();ctx.textAlign='right';ctx.fillText(fmtLAxis(v),pad.l-7,yy)});[0,100,200,300].forEach(function(v){ctx.textAlign='left';ctx.fillStyle=col('--dim');ctx.fillText(fmtRAxis(v),w-pad.r+7,yR(v))});
  var bw=Math.max(7,Math.min(21,pw/d.years.length*.24));function stacked(key,i,xx,scale){var base=0,total=d[key][i];if(total==null)return;d.companies.forEach(function(c){var share=c.share[key][i];if(share==null)return;var val=total*share,top=scale(base+val),bottom=scale(base);ctx.fillStyle=c.color;ctx.fillRect(xx,top,bw,bottom-top);base+=val})}d.backlog.forEach(function(v,i){var xx=x(i)-bw*1.5-2;if(v==null){ctx.fillStyle=col('--faint');ctx.textAlign='center';ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText('없음',x(i)-bw-2,Math.max(18,yL(0)-12));return}var yy=yL(v),ly=Math.max(12,yy-20);stacked('backlog',i,xx,yL);ctx.fillStyle=col('--txt');ctx.textAlign='center';ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText(fmtLValue(v),x(i)-bw-2,ly);ctx.font='10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText('('+((v/(d.revenue[i]*4)).toFixed(1))+')',x(i)-bw-2,ly+11)});
  d.revenue.forEach(function(v,i){var xx=x(i)-bw/2,yy=yR(v);stacked('revenue',i,xx,yR);ctx.fillStyle=col('--st-hot');ctx.textAlign='center';ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');var ly=Math.max(12,yy-20);ctx.fillText(fmtRValue(v),x(i),ly);ctx.font='10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText('('+d.opMargin[i]+')',x(i),ly+11)});
  d.capex.forEach(function(v,i){var xx=x(i)+bw/2+2,yy=yR(v);stacked('capex',i,xx,yR);ctx.fillStyle=col('--st-mature');ctx.textAlign='center';ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');var ly=Math.max(12,yy-20);ctx.fillText(fmtRValue(v),x(i)+bw+2,ly);ctx.font='10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText('('+Math.round(v/d.revenue[i]*100)+')',x(i)+bw+2,ly+11)});
...
function mountAnnualCloudChart(root){
 ...
 function draw(){var r=box.getBoundingClientRect(),dpr=Math.min(window.devicePixelRatio||1,2),w=Math.max(290,Math.round(r.width)),h=Math.max(240,Math.round(r.height));canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);
  var pad={l:48,r:48,t:30,b:40},pw=w-pad.l-pad.r,ph=h-pad.t-pad.b,leftMax=3000,rightMax=900,x=function(i){return pad.l+pw*i/(d.years.length-1)},yL=function(v){return pad.t+ph*(1-v/leftMax)},yR=function(v){return pad.t+ph*(1-v/rightMax)};
  ...
  function line(vals,scale,color){ctx.strokeStyle=col(color);ctx.lineWidth=2.2;ctx.lineJoin='round';ctx.lineCap='round';ctx.beginPath();vals.forEach(function(v,i){var xx=x(i),yy=scale(v);if(i===0)ctx.moveTo(xx,yy);else ctx.lineTo(xx,yy)});ctx.stroke();vals.forEach(function(v,i){var xx=x(i),yy=scale(v);ctx.fillStyle=col('--panel');ctx.strokeStyle=col(color);ctx.lineWidth=2;ctx.beginPath();ctx.arc(xx,yy,4,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle=col(color);ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.textAlign='center';ctx.fillText(Math.round(v),xx,Math.max(12,yy-11))})}
  ...
  line(d.rpo,yL,'--dawn');line(d.revenue,yR,'--st-hot');line(d.capex,yR,'--st-mature');
 ...
 function show(e){var r=canvas.getBoundingClientRect(),px=e.clientX-r.left;active=Math.max(0,Math.min(d.years.length-1,Math.round((px-48)/(r.width-96)*(d.years.length-1))));var i=active;tip.innerHTML='<b>'+d.years[i]+'</b><br>RPO · '+fmt(d.rpo[i])+'<br>Cloud 매출 · '+fmt(d.revenue[i])+'<br>CAPEX · '+fmt(d.capex[i])+'<br>RPO/매출 · '+(d.rpo[i]/d.revenue[i]).toFixed(1)+'배'+(i>=2?'<br><span style="color:var(--st-mature)">Base Case 전망</span>':'<br>실제');tip.style.display='block';tip.style.left=Math.min(r.width-170,Math.max(4,px+10))+'px';tip.style.top='8px';draw()}
 canvas.addEventListener('pointermove',show);canvas.addEventListener('pointerdown',show);canvas.addEventListener('pointerleave',function(e){if(e.pointerType!=='touch'){active=-1;tip.style.display='none';draw()}});var ro=typeof ResizeObserver!=='undefined'?new ResizeObserver(draw):null;if(ro)ro.observe(box);else window.addEventListener('resize',draw);draw();
}
function mount(){
