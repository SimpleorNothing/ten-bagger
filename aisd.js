...
function mountQuarterlyCloudChart(root){
 ...
 function draw(){var r=box.getBoundingClientRect(),dpr=Math.min(window.devicePixelRatio||1,2),w=Math.max(290,Math.round(r.width)),h=Math.max(220,Math.round(r.height));canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);
  ...
  var bw=Math.max(7,Math.min(21,pw/d.years.length*.24));d.backlog.forEach(function(v,i){var xx=x(i)-bw*1.5-2,yy=yL(v),ly=Math.max(12,yy-20);ctx.fillStyle=col('--dawn');ctx.fillRect(xx,yy,bw,yL(0)-yy);ctx.fillStyle=col('--txt');ctx.textAlign='center';ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText(fmtLValue(v),x(i)-bw-2,ly);ctx.font='10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText('('+((v/(d.revenue[i]*4)).toFixed(1))+')',x(i)-bw-2,ly+11)});
  d.revenue.forEach(function(v,i){var xx=x(i)-bw/2,yy=yR(v);ctx.fillStyle=col('--st-hot');ctx.fillRect(xx,yy,bw,yR(0)-yy);ctx.fillStyle=col('--st-hot');ctx.textAlign='center';ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');var ly=Math.max(12,yy-20);ctx.fillText(fmtRValue(v),x(i),ly)});
  d.capex.forEach(function(v,i){var xx=x(i)+bw/2+2,yy=yR(v);ctx.fillStyle=col('--st-mature');ctx.fillRect(xx,yy,bw,yR(0)-yy);ctx.fillStyle=col('--st-mature');ctx.textAlign='center';ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');var ly=Math.max(12,yy-20);ctx.fillText(fmtRValue(v),x(i)+bw+2,ly);ctx.font='10px '+getComputedStyle(root).getPropertyValue('--mono');ctx.fillText('('+Math.round(v/d.revenue[i]*100)+')',x(i)+bw+2,ly+11)});
 ...
 function show(e){var r=canvas.getBoundingClientRect(),px=e.clientX-r.left;active=Math.max(0,Math.min(d.years.length-1,Math.round((px-47)/(r.width-63)*d.years.length-.5)));var i=active;tip.innerHTML='<b>'+d.years[i]+'</b><br>수주잔고 · '+fmtLValue(d.backlog[i])+'<br>연환산 매출 대비 · '+(d.backlog[i]/(d.revenue[i]*4)).toFixed(1)+'배<br>매출 · '+fmtRValue(d.revenue[i])+'<br>CAPEX · '+fmtRValue(d.capex[i])+'<br>('+Math.round(d.capex[i]/d.revenue[i]*100)+')';tip.style.display='block';tip.style.left=Math.min(r.width-164,Math.max(4,px+10))+'px';tip.style.top='7px';draw()}
