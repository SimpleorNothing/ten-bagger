...
<div class="ds-topcharts" aria-label="컴퓨팅 판매자 핵심 지표 차트">
  <section class="ds-topchart wide">
    ...
    <div class="ds-topfn">좌측 축=수주잔고(최대 2.00T · 0.50T 간격) · 우측 축=클라우드 서비스 매출·CAPEX(최대 300B · 100B 간격) · 막대=수주잔고·매출·CAPEX · 매출 괄호는 수주잔고 대비, CAPEX 괄호는 매출 대비 · Azure 매출은 공시 성장률 기반 환산, AWS 수주잔고는 1Q26 기준(신규 OpenAI $100B 제외)</div>
  </section>
</div>
...
function mountQuarterlyCloudChart(root){
 ...
 var d={years:['24.1Q','24.2Q','24.3Q','24.4Q','25.1Q','25.2Q','25.3Q','25.4Q','26.1Q','26.2Q'],backlog:[400,475,550,650,748,856,1011,1190,1451,1556],revenue:[52,58,65,74,81,88,96,107,123,140],capex:[36,45,52,61,68,75,88,105,145,150],companies:[{name:'Microsoft',color:'#4e79a7',share:{backlog:[.47,.47,.47,.47,.48,.48,.48,.48,.47,.436],revenue:[.38,.38,.38,.38,.39,.39,.39,.39,.39,.39],capex:[.31,.31,.31,.31,.32,.32,.32,.32,.30,.28]}},{name:'Amazon',color:'#f28e2b',share:{backlog:[.30,.30,.30,.30,.29,.29,.29,.29,.28,.234],revenue:[.42,.42,.42,.42,.41,.41,.41,.41,.40,.39],capex:[.42,.42,.42,.42,.41,.41,.41,.41,.43,.39]}},{name:'Alphabet',color:'#59a14f',share:{backlog:[.23,.23,.23,.23,.23,.23,.23,.23,.25,.330],revenue:[.20,.20,.20,.20,.20,.20,.20,.20,.21,.22],capex:[.27,.27,.27,.27,.27,.27,.27,.27,.27,.33]}}]},ctx=canvas.getContext('2d'),active=-1;
 ...
 function draw(){var r=box.getBoundingClientRect(),dpr=Math.min(window.devicePixelRatio||1,2),w=Math.max(290,Math.round(r.width)),h=Math.max(220,Math.round(r.height));canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);
  ...
  d.revenue.forEach(function(v,i){var xx=x(i)-bw/2,yy=yR(v);stacked('revenue',i,xx,yR);ctx.fillStyle=col('--st-hot');ctx.textAlign='center';ctx.font='700 10px '+getComputedStyle(root).getPropertyValue('--mono');var ly=Math.max(12,yy-20);ctx.fillText(fmtRValue(v),x(i),ly)});
 ...
 function lines(key,i){return d.companies.map(function(c){return '<span style="color:'+c.color+'">■</span> '+c.name+' '+fmtRValue(Math.round(d[key][i]*c.share[key][i]))}).join('<br>')} function show(e){var r=canvas.getBoundingClientRect(),px=e.clientX-r.left;active=Math.max(0,Math.min(d.years.length-1,Math.round((px-47)/(r.width-63)*d.years.length-.5)));var i=active;tip.innerHTML='<b>'+d.years[i]+'</b><br>수주잔고 · '+fmtLValue(d.backlog[i])+'<br>'+lines('backlog',i)+'<br>연환산 매출 대비 · '+(d.backlog[i]/(d.revenue[i]*4)).toFixed(1)+'배<br>매출 · '+fmtRValue(d.revenue[i])+'<br>'+lines('revenue',i)+'<br>CAPEX · '+fmtRValue(d.capex[i])+'<br>'+lines('capex',i)+'<br>('+Math.round(d.capex[i]/d.revenue[i]*100)+')';tip.style.display='block';tip.style.left=Math.min(r.width-164,Math.max(4,px+10))+'px';tip.style.top='7px';draw()}
