...
<div class="ds-topcharts" aria-label="컴퓨팅 판매자 핵심 지표 차트">
  <section class="ds-topchart wide">
    ...
    <div class="ds-topfn"><span style="color:#315b78">■ Microsoft</span> · <span style="color:#5f86a0">■ Amazon</span> · <span style="color:#9bb5c5">■ Alphabet</span> · 좌측 축=공개 수주잔고(최대 2.00T · 0.50T 간격) · 우측 축=클라우드 서비스 매출·CAPEX(최대 300B · 100B 간격) · 매출·CAPEX는 3사 합산, 수주잔고는 각사 분기 공시액만 합산(AWS는 26.1Q RPO $364B만 공개) · 수주잔고 괄호=3사 연환산 매출 대비 배수 · 매출 괄호=3사 매출 가중 영업이익률(Microsoft는 Intelligent Cloud 부문 실적 기준)</div>
  </section>
</div>
...
function mountQuarterlyCloudChart(root){
 ...
 // 수주잔고=Microsoft 상업 RPO·Google Cloud revenue backlog·AWS RPO의 실제 분기 공시액만 합산(AWS는 26.1Q $364B만 공시).
 // CAPEX=각사 분기 실적발표의 현금 CAPEX·금융리스 포함 인프라 투자 기준. 영업이익률=AWS·Google Cloud와 Microsoft Intelligent Cloud의 공시 부문 수치를 3사 클라우드 매출로 가중. 단위: $B.
 var d={years:['24.1Q','24.2Q','24.3Q','24.4Q','25.1Q','25.2Q','25.3Q','25.4Q','26.1Q','26.2Q'],backlog:[264,327,333,391,413,474,547,865,1455,1192],revenue:[70,73,78,82,84,91,97,105,114,126],capex:[36,45,52,61,68,75,88,105,145,150],opMargin:[35,34,36,35,37,35,36,40,40,39],companies:[{name:'Microsoft',color:'#315b78',share:{backlog:[.81,.82,.78,.76,.76,.78,.72,.72,.431,.57],revenue:[.503,.501,.500,.501,.505,.514,.505,.491,.479,.470],capex:[.31,.31,.31,.31,.32,.32,.32,.32,.30,.28]}},{name:'Amazon',color:'#5f86a0',share:{backlog:[0,0,0,0,0,0,0,0,.250,0],revenue:[.357,.360,.354,.352,.349,.340,.339,.340,.334,.335],capex:[.42,.42,.42,.42,.41,.41,.41,.41,.43,.39]}},{name:'Alphabet',color:'#9bb5c5',share:{backlog:[.19,.18,.22,.24,.24,.22,.28,.28,.319,.43],revenue:[.140,.139,.146,.147,.146,.146,.156,.169,.187,.195],capex:[.27,.27,.27,.27,.27,.27,.27,.27,.27,.33]}}]},ctx=canvas.getContext('2d'),active=-1;
 ...
 function lines(key,i){return d.companies.map(function(c){return '<span style="color:'+c.color+'">■</span> '+c.name+' '+fmtRValue(Math.round(d[key][i]*c.share[key][i]))}).join('<br>')} function show(e){var r=canvas.getBoundingClientRect(),px=e.clientX-r.left;active=Math.max(0,Math.min(d.years.length-1,Math.round((px-47)/(r.width-63)*d.years.length-.5)));var i=active;tip.innerHTML='<b>'+d.years[i]+'</b><br>수주잔고 · '+fmtLValue(d.backlog[i])+'<br>'+lines('backlog',i)+'<br>연환산 매출 대비 · '+(d.backlog[i]/(d.revenue[i]*4)).toFixed(1)+'배<br>매출 · '+fmtRValue(d.revenue[i])+'<br>'+lines('revenue',i)+'<br>영업이익률 가중평균 · '+d.opMargin[i]+'<br>CAPEX · '+fmtRValue(d.capex[i])+'<br>'+lines('capex',i)+'<br>('+Math.round(d.capex[i]/d.revenue[i]*100)+')';tip.style.display='block';tip.style.left=Math.min(r.width-164,Math.max(4,px+10))+'px';tip.style.top='7px';draw()}
