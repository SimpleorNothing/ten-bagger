...
<div class="ds-vct t3">
  ...
  <div class="ds-vces">
    <span><span class="k">Oracle FY27 Q1 · OCI <b>$7.4B(+121%)</b> · RPO <b>$664B</b> · CAPEX <b>$28.499B</b> · FCF <b>-$5.396B</b></span>
...
function mountAnnualCloudChart(root){
 ...
 function draw(){var r=box.getBoundingClientRect(),dpr=Math.min(window.devicePixelRatio||1,2),w=Math.max(290,Math.round(r.width)),h=Math.max(240,Math.round(r.height));canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);
  ...
  line(d.rpo,'--dawn',0,-12);
  // 같은 연도에서는 큰 값의 라벨을 위쪽에, 작은 값의 라벨을 아래쪽에 배치한다.
  line(d.revenue,'--st-hot',-8,function(v,i){return d.revenue[i]>=d.capex[i]?-20:20});line(d.capex,'--st-mature',8,function(v,i){return d.capex[i]>=d.revenue[i]?-20:20});
