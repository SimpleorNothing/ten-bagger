(function(){
  ...
  var COLORS={total:'phc-total',personal:'phc-personal',dc:'phc-dc',irp:'phc-irp'};
  var LABELS={total:'전체',personal:'개인투자',dc:'DC연금',irp:'IRP'};
  var ORDER=['total','personal','dc','irp'];
...
#portfolioHistoryChart{margin:0 0 14px;border:1px solid var(--line);background:var(--panel)}.phc-hd{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;padding:13px 14px 9px;flex-wrap:wrap}.phc-title{font-size:14px;font-weight:800;color:var(--txt)}.phc-sub{font-size:11px;line-height:1.5;color:var(--faint);margin-top:3px}.phc-ranges{display:flex;gap:5px;flex-wrap:wrap}.phc-ranges button{border:1px solid var(--line2);background:var(--panel2);color:var(--dim);font:700 10px var(--sans);padding:5px 8px;border-radius:4px;cursor:pointer}.phc-ranges button.on{background:var(--txt);color:var(--ink2)}.phc-legend{display:flex;gap:14px;align-items:center;flex-wrap:wrap;padding:0 14px 8px;font-size:11px;color:var(--dim)}.phc-legend i{display:inline-block;width:16px;height:3px;border-radius:3px;margin-right:5px;vertical-align:middle}.phc-svgwrap{position:relative;padding:0 8px 10px}.phc-svg{width:100%;height:285px;display:block;overflow:visible}.phc-grid{stroke:var(--line);stroke-width:1}.phc-axis{fill:var(--faint);font:10px var(--sans)}.phc-path{fill:none;stroke-width:2.2;vector-effect:non-scaling-stroke}.phc-total{stroke:var(--txt)}.phc-personal{stroke:var(--dawn)}.phc-dc{stroke:var(--st-mature)}.phc-irp{stroke:var(--st-hot)}.phc-dot-total{background:var(--txt)}.phc-dot-personal{background:var(--dawn)}.phc-dot-dc{background:var(--st-mature)}.phc-dot-irp{background:var(--st-hot)}.phc-cross{stroke:var(--faint);stroke-width:1;stroke-dasharray:3 3}.phc-hit{fill:transparent;cursor:crosshair}.phc-tip{position:absolute;display:none;z-index:3;min-width:180px;padding:9px 10px;border:1px solid var(--line2);background:var(--ink2);box-shadow:0 8px 25px rgba(0,0,0,.16);font-size:11px;line-height:1.6;pointer-events:none}.phc-tip b{display:block;color:var(--txt);font-size:12px;margin-bottom:2px}.phc-tip-row{display:flex;justify-content:space-between;gap:14px}.phc-loading,.phc-empty{padding:36px 14px;text-align:center;color:var(--faint);font-size:12px}@media(max-width:650px){.phc-svg{height:240px}.phc-sub{max-width:280px}.phc-legend{gap:8px}}';document.head.appendChild(st);}
  ...
  function render(data){
    ...
    var W=960,H=285,L=66,R=18,T=12,B=34;var x=function(ms){return L+(ms-minX)/(maxX-minX)*(W-L-R);};var y=function(v){return T+(1-v/maxY)*(H-T-B);};
  ...
  async function load(){
    ...
    try{
      var seed=await fetchJson(SEED),personal=mapFromPairs(seed&&seed.series&&seed.series.personal,'첨부 원장'),dc=mapFromPairs(seed&&seed.series&&seed.series.dc,'첨부 원장'),irp=mapFromPairs(seed&&seed.series&&seed.series.irp,'첨부 원장');
      ...
      allSeries={personal:sortedValues(personal),dc:sortedValues(dc),irp:sortedValues(irp)};allSeries.total=buildTotal(personal,dc,irp);render(allSeries);
    }catch(e){if(body)body.innerHTML='<div class="phc-empty">보유현황 그래프를 불러오지 못했습니다. '+esc(e&&e.message||e)+'</div>';}
  }

  function mount(){
    if(mounted||document.getElementById('portfolioHistoryChart'))return true;var note=document.getElementById('acNote');if(!note||!note.parentNode)return false;mounted=true;injectStyle();var sec=document.createElement('section');sec.id='portfolioHistoryChart';sec.innerHTML='<div class="phc-hd"><div><div class="phc-title">보유자산 평가금액 추이</div><div class="phc-sub">개인투자 2023~2025 과거원장 · 2026 NHPLUG 우선(API 저장 전 구간은 2026 원장 백필) · DC/IRP 첨부·수동 스냅샷</div></div><div class="phc-ranges"><button data-r="6M">6M</button><button data-r="1Y">1Y</button><button data-r="3Y">3Y</button><button class="on" data-r="ALL">ALL</button></div></div><div class="phc-legend"><span><i class="phc-dot-total"></i>전체</span><span><i class="phc-dot-personal"></i>개인투자</span><span><i class="phc-dot-dc"></i>DC연금</span><span><i class="phc-dot-irp"></i>IRP</span></div><div class="phc-svgwrap" id="phcBody"><div class="phc-loading">그래프 준비 중…</div></div>';
