...
pyfedwatch only if neither verified source is usable. Unlike the legacy runner,
this preserves the source's actual Updated date/time and accepts the latest
Friday/holiday snapshot for up to 3 calendar days instead of pretending the
snapshot belongs to the automation run date.
"""
...
def save_verified(doc, source, source_url, ranges, source_date, meta, priority):
    ...
    checks.append({
        "date": today,
        "checkedAt": checked,
        "status": f"확인됨({priority}순위)",
        "source": source,
        "sourceDate": source_date,
        "sourceUpdatedAt": source_updated,
        "reason": (
            f"{source} 검증값 사용. 실제 원문 Updated={source_updated}; "
            f"자동화 실행일을 기준일로 승격하지 않고 원문 기준일 {source_date}를 유지."
        ),
        "inputs": meta or {},
    })
...
def sync_pulse(doc):
    ...
    updated = h.get("sourceUpdatedAt") or source_date
...
def sync_changelog(doc):
    ...
    if not r:
        return
    probs = "/".join(f"{float(x['probability']):.1f}" for x in r)
    fp = h.get("futurePrice")
    updated = h.get("sourceUpdatedAt") or h.get("sourceDate") or h.get("date")
    entry = (
        "    {d:'2026-09-07',t:'01 FedWatch 원문시각 정합성 — CME 공식 EOD 직접값 미확보 시 Investing.com 표시값을 2순위로 사용하되 자동화 실행일이 아니라 실제 Updated 시각을 보존. "
        f"12월 확률 {probs}%" + (f", 선물 {fp:.3f}" if fp is not None else "") + f", 원문 Updated {updated} 기준으로 fedwatch·시장맥박을 동기화'},\n"
    )
...
def main():
    ...
    sources = [
        ...
        ("Investing.com Fed Rate Monitor", fw.INVESTING_URL, fw.fetch_investing, 2),
