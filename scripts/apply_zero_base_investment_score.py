...
s = re.sub(r'\n?<!-- ZERO_BASE_INVESTMENT_SCORE_V[123] -->\n<script>.*?</script>\n?', '\n', s, flags=re.S)

script = r'''
<!-- ZERO_BASE_INVESTMENT_SCORE_V3 -->
<script>
(function(){
 const ZB={gamma:null,holdings:null,benchmark:null};
 const clamp=(v,a,b)=>Math.max(a,Math.min(b,v));
 const scoreRange=(v,lo,hi)=>v==null?null:clamp((v-lo)/(hi-lo)*100,0,100);
 const revisionScore=(key,v)=>{const m=ZB.benchmark&&ZB.benchmark.metrics&&ZB.benchmark.metrics[key];return v==null||!m||m.n<(ZB.benchmark.minimumSample||50)||!(m.p10<m.p90)?null:scoreRange(v,m.p10,m.p90);};
 const revisionAnchor=(key)=>ZB.benchmark&&ZB.benchmark.metrics&&ZB.benchmark.metrics[key];
 ...
 function calc(G){
   if(!G||!ZB.benchmark||ZB.benchmark.schema!=='revision-benchmark-v1')return {score:null,coverage:0,penalty:0,detail:{}};
   ...
   const s3=weighted([
     [revisionScore('q1_c30',q1.c30),50],
     [revisionScore('q1_c90',q1.c90),20],
     [revisionScore('fy1_c30',fy1.c30),30]
   ]);
   const s6=weighted([
     [scoreRange(epsGrowth,-10,60),60],
     [revisionScore('fy1_c90',fy1.c90),25],
     [revisionScore('fy1_c30',fy1.c30),15]
   ]);
   const val=weighted([
   ...
   const cat=weighted([
     [revisionScore('q1_c30',q1.c30),25],
     [scoreRange(tp.c30,-5,10),20],
     [gammaScore(G.g),25],
   ...
   if(q1.c30!=null&&q1.c30<0){const a=revisionAnchor('q1_c30');if(a&&a.p10<0)penalty+=clamp(-q1.c30/Math.abs(a.p10)*4,0,4);}
   if(fy1.c30!=null&&fy1.c30<0){const a=revisionAnchor('fy1_c30');if(a&&a.p10<0)penalty+=clamp(-fy1.c30/Math.abs(a.p10)*4,0,4);}
 ...
 function renderBenchmarkNote(root,table){
   const b=ZB.benchmark,m=b&&b.metrics;if(!m)return;
   let note=root.querySelector('[data-zb-benchmark-note]');
   if(!note){note=document.createElement('div');note.dataset.zbBenchmarkNote='1';table.parentNode.insertBefore(note,table);}
   const ns=Object.values(m).map(x=>Number(x&&x.n)).filter(Number.isFinite),nMin=ns.length?Math.min(...ns):null,nMax=ns.length?Math.max(...ns):null;
   note.style.cssText='margin:0 0 8px;padding:7px 10px;border:1px solid var(--line);border-radius:7px;background:var(--panel);font-size:10.5px;color:var(--dim);line-height:1.45';
   note.innerHTML='<b style="color:var(--txt)">EPS 리비전 점수 기준</b> · Nasdaq-100 당일 분포 P10=0점 · P90=100점 · 기준 '+String(b.asOf||'').slice(0,10)+' · 유효표본 '+(nMin==null?'—':nMin+(nMax!==nMin?'~'+nMax:''))+'개';
 }
 function render(){
   ...
   renderBenchmarkNote(root,table);
   ...
   Array.from(table.querySelectorAll('tbody tr')).forEach((tr,order)=>{
     ...
     const title=[
       ...
       '',
       '최종 = 가중 기초점수 - 리스크 페널티',
       'EPS 리비전 환산 = Nasdaq-100 당일 횡단면 P10 0점 · P90 100점 (구성 '+(ZB.benchmark.constituentCount||'—')+'개, 기준 '+String(ZB.benchmark.asOf||'').slice(0,10)+')',
       '실제 기준: Q1 30d '+revisionAnchor('q1_c30').p10+'~'+revisionAnchor('q1_c30').p90+'% · Q1 90d '+revisionAnchor('q1_c90').p10+'~'+revisionAnchor('q1_c90').p90+'% · FY+1 30d '+revisionAnchor('fy1_c30').p10+'~'+revisionAnchor('fy1_c30').p90+'% · FY+1 90d '+revisionAnchor('fy1_c90').p10+'~'+revisionAnchor('fy1_c90').p90+'%',
 ...
 async function boot(){
   try{const rr=await Promise.all([fetch('./gamma.json',{cache:'no-store'}).then(r=>r.ok?r.json():null).catch(()=>null),fetch('./holdings.json',{cache:'no-store'}).then(r=>r.ok?r.json():null).catch(()=>null),fetch('./revision-benchmark.json',{cache:'no-store'}).then(r=>r.ok?r.json():null).catch(()=>null)]);ZB.gamma=rr[0];ZB.holdings=rr[1];ZB.benchmark=rr[2];}catch(e){}
