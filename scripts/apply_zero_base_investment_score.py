...
(function(){
 ...
 function note(table){
   ...
   el.innerHTML='<b style="color:var(--txt)">V4.1 투자매력도</b> · Nasdaq-100 순위 백분위(P50=50) · 외생 데이터 점수와 내부 판단 보정 분리 · 기준 '+String(b.asOf||'').slice(0,10)+' · 유효표본 '+(ns.length?Math.min(...ns)+'~'+Math.max(...ns):'—')+'개';
 }
 function render(){
   ...
   const th=document.createElement('th');th.dataset.zbScore='1';th.innerHTML='투자매력도<br><small style="font-weight:400;color:var(--faint)">V4.1 데이터+판단 · 100</small>';head.insertBefore(th,head.children[stockIdx+1]||null);
   ...
   Array.from(body.querySelectorAll('tr')).forEach((tr,order)=>{
     ...
     if(ZB.error)td.innerHTML='<b style="color:var(--st-hot)">산출 실패</b><div class="pe-p">scores.json 확인</div>';
       ...
       const d=v.dimensions||{},adj=Number(v.adjustment||0),opp=Number(v.opportunityAdjustment||0),fd=(v.inputs&&v.inputs.fundamentalDislocation)||{},rank=ticker&&held[ticker]?'<span data-zb-rank style="font-weight:700;color:var(--dim)"></span>':'<span style="font-size:10px;color:var(--faint)">후보</span>';
       const oppTxt=opp>0?' · 급락기회 +'+fmt(opp,1):'';
       td.innerHTML='<div style="display:flex;gap:6px;align-items:baseline;white-space:nowrap">'+rank+'<b style="font-size:19px;color:'+tone(v.score)+'">'+v.score+'</b><span style="font-size:10px;color:var(--faint)">'+v.label+'</span></div><div style="font-size:9.5px;color:var(--dim);white-space:nowrap">리비전 '+fmt(d.revisionMomentum)+' · 성장 '+fmt(d.growth)+' · 밸류 '+fmt(d.valuation)+'</div><div style="font-size:9.5px;color:var(--faint);white-space:nowrap">괴리 '+fmt(d.estimateLead)+oppTxt+' · 판단 '+(adj>0?'+':'')+fmt(adj)+' · '+fmt(v.coverage*100)+'%</div>';
       td.title=['V4 최종 '+v.score+' = 외생 데이터 '+fmt(v.dataScore,1)+' + 내부 판단 '+(adj>0?'+':'')+fmt(adj,1),'리비전 모멘텀 '+fmt(d.revisionMomentum,1)+' · EPS 성장 '+fmt(d.growth,1)+' · 실제 밸류 '+fmt(d.valuation,1)+' · 컨센서스 '+fmt(d.consensus,1)+' · 가격-EPS 괴리 '+fmt(d.estimateLead,1),'급락기회 '+(opp>0?('+'+fmt(opp,1)+'점 · 최근 7일 최악 일간 '+fmt(fd.shock7dWorst1d,1)+'% · 급락 전 대비 '+fmt(fd.residualFromPreShock,1)+'% · 펀더멘털 보존 통과'):'0점 · 성장성/급락폭/EPS·목표가·컨센서스 보존 조건 미충족'),'판단 보정: γ '+(v.adjustmentDetail.gamma>0?'+':'')+fmt(v.adjustmentDetail.gamma,1)+' · 사이클 '+(v.adjustmentDetail.stage>0?'+':'')+fmt(v.adjustmentDetail.stage,1),'절대 EPS 상태: '+v.inputs.absoluteRevision+' · FY+1 EPS30 '+fmt((ZB.scores.rows[ticker].v4.inputs||{}).fy1c30,1),'기존 V3 '+fmt(old&&old.score)+' · 변화 '+(snap.delta>0?'+':'')+fmt(snap.delta),'모델 '+ZB.scores.modelVersion+' · 스냅샷 '+String(ZB.scores.asOf||'')].join('\n');
     }
