...
const stocks = unique.filter((ticker) => gamma[ticker]?.instrumentType !== 'ETF');
const etfs = unique.filter((ticker) => gamma[ticker]?.instrumentType === 'ETF');
const missingGamma = unique.filter((ticker) => !gamma[ticker]);
const missingScores = stocks.filter((ticker) => !scores[ticker]);
const includedEtfs = etfs.filter((ticker) => scores[ticker]);
...
if (includedEtfs.length) throw new Error(`ETF must be excluded from scores: ${includedEtfs.join(', ')}`);

console.log(`Value Chain stock investment-score coverage OK: ${stocks.length}/${stocks.length} · ETF excluded: ${etfs.length}`);
