...
function leaderPoints(map){
  const crclOk=finite(map.crcl?.changePct),btcOk=finite(map.btc?.changePct);
  const crcl=crclOk?Number(map.crcl.changePct):NaN,btc=btcOk?Number(map.btc.changePct):NaN;
  const peerValues=['crcl','coin','hood'].filter(k=>finite(map[k]?.changePct));
  const peers=peerValues.filter(k=>Number(map[k].changePct)>=7).length;
  const core=crclOk?(crcl>=10?3:crcl>=7?2:crcl>=4?1:0):0;
  return {
    core:{label:'CRCL 핵심선행',available:crclOk,points:core,max:3},
    btc:{label:'BTC +4%',available:btcOk,points:btcOk&&btc>=4?1:0,max:1},
    peers:{label:'동종 2개 +7%',available:peerValues.length>=2,points:peerValues.length>=2&&peers>=2?1:0,max:1}
  };
}
...
for(const c of CANDIDATES){
  ...
  candidates.push({code:c.code,name:c.name,score:sumPoints(components),maxScore:MAX_SCORE,coveredMaxScore,recent5dPct:hist.recent5dPct,lastPrice:hist.lastPrice,linkage:c.linkage,components,sources:[hist.source,c.catalyst?.source].filter(Boolean)});
  await sleep(250);
}

const availableMax=candidates.length?Math.min(...candidates.map(c=>c.coveredMaxScore)):0;
const leaders=LEADERS.map(x=>leaderMap[x.id]);
const top=Math.max(0,...candidates.map(x=>x.score));
const doc={
  schema:'momentum-radar-v1',
  asOf:kstIso(),
  status:leaders.every(x=>finite(x.changePct))?'auto-verified':'partial',
  maxScore:MAX_SCORE,
