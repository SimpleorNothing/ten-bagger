...
- 2026-08-01 07:45 · **03 전문가 원탁 토론 이력은 최근 1건만 유지.** 이력 섹션의 기존 `h2.msec`·`.mnote`·`.cl-btn`과 삭제/음성 재생 UI를 유지하고 안내만 「최근 생성 1건만 유지」로 정정했다. 서버 저장 정책 변경이며 신규 면·CSS 클래스·`:root` 토큰 0, 기존 3px 면·12px 메타·14px 본문 규약은 불변. (OPS §3 동반)
