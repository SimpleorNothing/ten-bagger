...
- 2026-07-17 20:02 · **03 라이프사이클 편집을 모달 + 「보기 칩」 선택식으로.** `window.prompt` 4연타 → 오버레이 모달 컴포넌트 신설(`.ins-lc-*`). 보기는 클라 템플릿(게이트 어휘·8레이어·관점 티커·thesis-break 패턴 즉시 생성 — 서버·외부호출 0). 칩 클릭=아래 칸 채우기(단일)·직접 수정 가능·Esc/배경/취소 닫기. **신규 `:root` 토큰 0** → TOKENS 무변·`check-docs` 통과. 면 radius 3px·칩 20px(§3)·폼 14px·메타 12px(§2 하한). `#v-insight` 스코프. SimpleorNothing 지시. (OPS §3·§8·§9 동반)
- 2026-07-17 18:56 · **06 캘린더 삭제 → 01 「다가오는 일정」 흡수.** SimpleorNothing 지시. nav `cal` 버튼 제거, 「임박 이벤트」 컴포넌트·`--cat-*`·3px목록을 `#v-cal`→`#v-market` 이관만(신규 클래스·토큰 0). v-cal은 v-port식 코드 잔존. §5·§6-1 갱신. Playwright 렌더 검증(nav 01~06·D-N 8·pageerror 0). (OPS §3·§9 동반)
- 2026-07-17 17:12 · **04 「관점 지형」(`#clSynth`)·「여러 링크」 소스별 ✕ 제외/복원.** 관점 지형은 council.json synthesis를 `.cl-*` 재사용 렌더. 소스 ✕/복원은 기존 `.cl-btn`(면 3px)·제외 행 opacity .4+취소선. **신규 토큰·클래스 0** → TOKENS 무변·check-docs 통과. (OPS §3·§9 동반)
- 2026-07-17 16:38 · **04 전문가 원탁에 「토론 주제」 입력창 추가.** 「현 상황」 위 단일행 `#clTopic` — 비우면 현 상황 종합, 채우면 그 논제 중심. **신규 `:root` 토큰·CSS 클래스 0** — 폼 `.cl-in` 재사용. TOKENS 무변 → `check-docs` 통과. narrative≠numbers. (OPS §3·§9 동반 갱신)
- 2026-07-17 13:38 · **04 전문가 원탁 관점 갱신 모달에 「여러 링크」 탭 추가.** 유튜브·기사 링크를 한꺼번에 붙여넣어 소스별 인식→통합 관점으로 정리. **신규 `:root` 토큰·CSS 클래스 0** — 기존 모달 컴포넌트(`.cl-*`)만 재사용. TOKENS 구역 무변 → `check-docs` 통과. (OPS §3·§9 동반 갱신)
- 2026-07-17 11:45 · **04 전문가 원탁 「음성 토론 재생」 플레이어 컴포넌트(`.cl-p*`).** 원탁 진단 리포트를 화자별 브라우저 TTS 메신저형으로 극화하는 오버레이. **신규 `:root` 토큰 0**(전역 토큰만) → TOKENS 구역 무변, `check-docs` 통과. 면=radius 3px·부표=radius 20px·아바타=기존 `avatar()` SVG 재사용. 말풍선 본문 14px·메타 12·13px(§2 하한 준수). `#v-council` 스코프. (OPS §3 동반 갱신)
...
- 2026-07-16 · **01 헤더 `update` 배지를 라이브 데이터 시각으로.** 배지 `update : YYYY.MM.DD`가 변경 로그(마지막 코드 수정일)라 데이터가 매 세션 갱신돼도 고착 → '업데이트 안 됨' 오독. `changelog.js`가 `pulse.json` asOf(KST 분단위) 페치해 `update : YYYY.MM.DD HH:MM` 표시, 변경 로그는 「이력 N」 클릭 모달로 이전. `index.html`·CSS·토큰 무변경(자가 마운트·신규 클래스 0·check-docs 무영향)·페치 실패 시 폴백. §4 갱신 배지 행 갱신. (PR #340)
- 2026-07-14 · **종목 뉴스 미니차트(`.stk-cv`) X축 날짜 연도 인식.** Ctrl+휠로 다년 창을 열면 시작·끝이 `MM-DD`만 찍혀 5년 차를 하루 차로 오독 → `fd()`를 `fd(x,yr)`+`fdD()`로 확장, `dr()`의 `spanYr`로 다년은 `YY-MM-DD`, 한 해 안은 종전 `MM-DD`. §6-5 갱신. 신규 토큰·CSS 없음(check-docs 무영향). (PR #333)
...
- 2026-07-12 23:59 · **03 관점 카드 출처 줄(`.ins-cs`).** 채택 관점 밑에 `출처: 매체 · 종류 · 날짜` + 링크(`.ins-cs-lk` — 원문 ↗ / 저장 원문 ↗)를 mono 12px `--faint`로 부착(메타 라벨 12px 하한 준수, §2). 링크는 `--dawn`·700, hover 밑줄. 저장 목록·다른 메뉴 스트립 공통. **신규 토큰 없음**(check-docs 무영향), `insight.css`에만 추가.
- 2026-07-12 23:15 · **03 관점과 정보 얻기 읽는 글을 `.vsub`(15px)에 정렬 + 최소 글자크기 규정 명문화(§2·§7).** insight.css의 폼·본문·요약·관점·스틸맨·저장목록 텍스트가 11.5~13.5px로 뷰 부제(15px)보다 작아 가독성 미달 → 읽는 글 전부 15px, 메타 라벨은 12px 하한으로 상향. insight.js 인라인 11px 버튼도 12px로. 폰트 크기는 토큰이 아니라 check-docs 무영향. §2에 폼 컨트롤·보조 뷰 하한, §7 체크리스트 11에 크기 하한 점검 추가.
- 2026-07-12 22:55 · **기간 버튼 5Y 실데이터화 — `fetch-prices.mjs` 창 1Y→5Y 확대.** Yahoo `range=1y→5y`, Naver `400일→1850일`(+상한 1300 캡). 매 실행 시리즈 전체 교체 → `update-prices` 워크플로가 다음 실행에서 charts.json을 5Y로 백필. 파일 크기 ~210KB→~1.1MB(gzip ~250KB). 프런트 무변경(count 기반 `slice6` 유효) — 단 `loadUs10y` `slice6` 적용해 라벨·창 일치. index.html은 JS 1줄만 → 신규 토큰·CSS 없음.
- 2026-07-12 23:55 · **01 카드 그래프 끝점 라벨 — 소수점 통일(fmtNum) + 선에서 더 띄움.** 끝점 라벨 소수점을 `fmtNum`(≥10 정수 콤마·<10 1자리)으로 통일. **헤더 `.mkt-val`은 정밀값 유지** — 그래프 라벨만 반올림. `.mkt-chart` padding-top 20→28 · `.spv` margin-top −4→−8. `chart()` 기본 포맷터도 fmtNum. 신규 토큰 없음(check-docs 무영향).
- 2026-07-12 23:45 · **01 시장 모니터링 스파크라인 세로 50% 확대(36→54px).** `.mkt-card .spark` height 36→54px · `chart()` 끝점 라벨 계산 `HH` 36→54 동기. 단일 `card()`/`chart()` 경로라 전 카드 자동 적용 · 신규 토큰 없음.
- 2026-07-12 23:40 · **종목 뉴스 차트 끝점 값 가독성 — 흰 헤일로 추가.** `halo()`(흰 외곽선 `lineWidth 3` 후 채움)로 모든 캔버스 라벨을 그려 라인 위에서도 읽히게 함. 위치·포맷 유지.
- 2026-07-12 23:05 · **종목 뉴스 차트: 처음·마지막 값을 각 끝점에 붙여 위/아래 표기.** `lblY()`로 끝점 위(−5)/상단 근접 시 아래(+12)에 배치, 처음=좌·마지막=우. 하단 눈금줄엔 날짜만. `fv()` 포맷·라인 색·볼드 유지.
- 2026-07-12 22:45 · **종목 뉴스 차트: 처음·마지막 값 하단 눈금줄 이동 + 수치 포맷 규칙.** `fv()` 신설 = **10 미만 소수 1자리 / 10 이상 정수(콤마)**, 처음·마지막·호버·평단 공통.
- 2026-07-12 19:35 · **01 끝점 수치 라벨 투명 배경 + 살짝 위로.** `.spv` 배경칩 제거(투명) · `margin-top:-4px` · `.mkt-chart` 상단 여백 16→20px.
- 2026-07-12 19:50 · **종목 뉴스 차트: 처음·마지막 값을 각 끝점 y 위로 이동 + 기간 증감률 좌상단.** 끝점 y는 `Y()`로 계산, 클램프로 겹침 방지.
- 2026-07-12 19:15 · **종목 뉴스 차트 값 라벨을 캔버스 상단 오버레이로 이전 + Ctrl+휠 안내 제거.** `.stk-cap` DOM 헤더 제거, `dr()`가 캔버스 상단 여백에 직접. `.stk-zn` 텍스트 삭제(휠 확대 유지).
- 2026-07-12 19:10 · **01 시장 모니터링: 카드 간 그래프 세로 위치 일치 + 끝점 수치 그래프 위로.** `.mkt-card` flex column·`.mkt-chart` `margin-top:auto`. X축의 시작·마지막 값을 `.spv` 오버레이로 이전, X축엔 날짜만. `chart()` 헬퍼 신설.
- 2026-07-12 18:20 · **종목 뉴스 그래프 카드 상단 정렬 + 처음·마지막 값 헤더.** `.stk-chart` `flex-start`. 값 라벨을 `.stk-cap` DOM 헤더로 이전(§6-5).
- 2026-07-12 18:30 · **01 스파크라인에 X축 시작·끝 라벨 추가.** `.spark` 아래 `.mkt-axis`(좌=시작일·시작값 / 우=끝값·마지막일). `card()`에 옵션 `{dates,fmtV}` 추가. 날짜 소스: charts.json `series.*.t` / WTI `points[i][0]`.
- 2026-07-12 17:45 · 빈 상태(§6-6) 보강 — 대기 중 렌즈 l1 유지 · `.stk-sum.ph` 대기 표기.
- 2026-07-12 17:44 · **05 알파맵에 렌즈 2줄(§6-4) 적용.** `.mkt-lens` 삽입: l1=벤치마크 프레임 / l2=사분면 분포·비중 + `MACRO_GRADE` 게이트→행동 판정. 버블 크기 토글(비중↔밴드갭) 툴바 `.tbtn` 재사용.
- 2026-07-12 17:40 · **03 관점 등급(승격) 배지·보드.** 관찰→후보→지지→확립→확신 5등급. `.ins-gr.g0~g4`·`.ins-tag.gpv`·`.ins-gboard`/`.ins-gcell`. 색은 기존 기능색 토큰 재사용 — **신규 토큰 없음**. `insight.css`에만 추가.
- 2026-07-12 17:20 · **01 시장 모니터링 카드에 전일대비 변동 병기.** `.mkt-dod`(전일 종가 대비 `pctDoD`) 추가. `card()` 단일 경로라 자동 적용. 데이터 2점 미만이면 미표시.
- 2026-07-12 15:54 · **타이포 스케일 리베이스(01).** 읽는 글 14px 하한·메타 12px 하한·블록 제목 15px·값 17px. `.mkt-grid` minmax 180→224px·`.mkt-card` min-height 104→128px. §2에 크기 하한 명문화.
- 2026-07-12 15:45 · 01 「관련 기사」 블록 **2열 그리드**(`#mktMacroNews`). `.stk-blk` 재사용·모바일 700px 이하 1열(§6-5).
- 2026-07-12 13:30 · 종목 뉴스 블록 우측 차트(`.stk-cv`) 높이 **190px 고정** 통일. 긴 블록은 `.stk-chart{justify-content:center}` 세로 중앙(§6-5).
- 2026-07-12 13:05 · v3.1 — 문서 맨 위에 **연월일시분(KST) 최종 갱신 타임스탬프** 도입(OPS 동일). §7 체크리스트 11항에 갱신 의무 반영.
- 2026-07-12 · **v3.** 지속 갱신 문서를 **2개(STYLE_GUIDE·OPS)** 로 확정 — 이 파일은 디자인 SoT. §3 해도 레이어, §4 전역 관행, §6 **01 시장 모니터링 레퍼런스 구현**, §7 **신규 메뉴 체크리스트 11항** 신설. 토큰 구역 무변경.
- 2026-07-12 · **v2 전면 재작성.** v1 6토큰(폐기 팔레트)을 라이브 실측 기준으로 교체, `scripts/check-docs.mjs` 드리프트 검출기 부착.
