...
- 2026-07-30 · **02 인사이트 「사이트 연관」 내부 근거 표시.** 관점 생성 전에 01 시장 모니터링·02 기존 채택 관점·03 전문가 원탁·04 시장/실적 전망·05 리밸런싱 컨텍스트를 자동 대조하고, 실제 사용된 근거만 claim `siteRefs[]`로 반환·저장한다. 추출 행과 채택 관점에서 기존 검증문 컴포넌트 `.ins-vf`를 재사용해 `메뉴 · 항목 (기준일) : 근거`를 표시한다. 신규 클래스·`:root` 토큰·CSS 0 → TOKENS 무변·check-docs 무영향. 기존 「반영하기」·라이프사이클·시그널 중첩 순서는 유지한다. (OPS §3·§9 동반)
