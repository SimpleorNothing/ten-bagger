...
> 2026-08-30 16:43 · **02 기업분석 전략 프레임 우측 분기 재무 막대그래프.** 기존 양면 프레임을 유지하고 우측 `.ca-frame-side` 안에 분기별 가로 막대그래프(`매출 / 영업이익`)를 배치한다. 두 계열은 기존 `--st-accel`·`--st-dawn`을 재사용하고, 실적은 실선·전망은 저채도 점선과 옅은 배경으로 구분한다. 공통 축·값 라벨·FY/CY 대응분기·산식 주석은 메타 하한 12px을 지키며, Q4E `미공시` 값도 자리표시자로 유지한다. 900px 이하에서는 기존 단일열 전환과 620px 차트 축소를 적용하며 신규 `:root` 토큰은 추가하지 않는다.
