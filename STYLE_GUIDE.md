...
| **매크로 토픽 접힘·이동** | 토픽 블록은 기본 접힘(`.is-collapsed`)이며 헤더 클릭 또는 Enter/Space로 펼친다(`aria-expanded`). 헤더의 `↕`는 마우스 드래그 핸들, `＋/−`는 상태 표시다. 드롭 후 축 키 순서를 `localStorage` `am_topic_radar_order_v1`에 저장해 새로고침 뒤에도 복원한다. 새 축은 저장된 축 뒤에 라이브 기본 순서로 추가된다. |
