...
> 2026-09-05 20:23 · **공통 내비게이션 안정성 규칙.** 메뉴 번호를 재계산하는 자가 마운트 모듈은 계산값이 기존 `textContent`와 다를 때만 DOM을 변경한다. `MutationObserver`가 감시하는 하위 노드를 무조건 다시 쓰지 않아야 하며, 신규 메뉴 추가 시 이 멱등 조건을 회귀 검사로 유지한다. 화면 디자인과 번호 체계는 변경하지 않는다.

> 2026-09-05 18:00 · **07 투자일지 신설.** `journal.js` 자가 마운트 뷰는 기존 `.vhead` 골격과 전역 `--panel`·`--line`·`--dawn` 토큰만 사용한다. 데스크톱은 판단 입력과 학습 대시보드 2열, 850px 이하 1열로 전환한다. 기록 목록은 모바일에서 날짜·종목·가설·상태만 남기고 EPS 상세·초과수익 열을 숨긴다. EPS 강화=`--st-dawn`, 약화=`--st-hot`, 중립=`--dim`; 신규 `:root` 토큰은 추가하지 않는다.

> 2026-09-01 · **02 기업분석 Broadcom 추가.** 기업 선택 탭에 `Broadcom · AVGO`를 등록하고 FY25 Q1~FY26 Q4E GAAP 분기 차트, 커스텀 XPU·Ethernet AI 네트워킹·VMware VCF·현금흐름·고객집중의 5개 전략축을 공통 기업분석 스키마로 표시한다. 공식 실적·SEC 자료에서 확인되지 않은 FY26 Q3 GAAP 영업이익과 FY26 Q4 전망은 `미공시`로 유지한다.

> 2026-08-30 20:35 · **02 기업분석 전략 프레임 우측 분기 재무 세로 막대그래프 — Marvell·Lumentum 공통.** 기존 양면 프레임을 유지하고 우측 `.ca-frame-side` 안에 두 기업의 CY25.1Q~CY26.4Q 대응 8개 분기 두 막대(`매출 / 영업이익`)를 나란히 배치한다. Lumentum은 FY25 Q3~FY26 Q4 실제 GAAP 값, FY27 Q1E 회사 매출 가이던스 중간값, FY27 Q2E 회사 가이던스 미공시 자리표시자를 사용한다. 상단은 색상 범례와 달력형 `분기별` 표시, 본문은 수평 격자·우측 GAAP·$B 눈금·압축 X축 라벨로 첨부 형태를 따른다. 두 계열은 기존 `--st-accel`·`--st-dawn`을 재사용하고, 실적은 단색·전망은 저채도 점선으로 구분한다. 산식 주석·`미공시` 접근성 라벨을 유지하고, 900px 이하 기존 단일열 전환과 620px 차트 축소를 적용한다. 신규 `:root` 토큰은 추가하지 않는다.

---

## 0. 토큰 체인 (읽는 순서)

> 2026-07-31 · 01~07 메뉴의 업데이트 배지는 기존 `.cyc-upd`·`.cyc-pop` 컴포넌트를 공통 사용한다. 배지는 `update : YYYY.MM.DD`만 노출하고 클릭 시 전체 변경 이력 팝업을 연다.
...
- **「오늘의 투자 명언」 스트립(2026-07-26 · `quote.js` 자가 마운트):** `#v-market` **첫 자식**(`.vhead` 위 = nav 바로 아래)에 `#mktQuote` 스트립을 끼운다(index.html 무편집 · 로더=changelog.js). 면 규약 = **radius 3px 직접 지정**(§3). 배경·테두리는 CNN F&G 50을 중립점으로 삼아 **0 방향은 청, 100 방향은 적**이며 극단값에 가까울수록 알파가 연속 증가한다. 값 미수집 시 `--panel`+`1px --line`으로 폴백한다. 명언 15px/600(`--txt`·따옴표 장식 `--dawn`) · 출처·레짐 칩 mono 12px `--faint`(§2 하한). 레짐 워드만 등락색 규약 준용 — 공포=하락측 청(`--st-accel`)·과열=상승측 적(`--st-hot`)·중립 무채(`--dim`), 기능색 `--st-*` 단계 의미로 쓰지 않는다. 클릭=같은 레짐 풀 안 랜덤 교체. ≤600px에서 칩은 줄바꿈 full-width. 신규 `:root` 토큰·전역 클래스 0(`<style id="quote-css">` 스코프).
...
- 2026-08-30 · **05 추정 리비전 트래커 「점수 기여도」 열 → 「손익비(R/R)」 열 교체.** 투자매력도 열 바로 오른쪽. 배수는 16px 굵게, `≥2` `--st-dawn`(양호)·`1~2` `--st-accel`(중립)·`<1` `--st-hot`(불리), 아래 `▲여력%`(`--st-dawn`)/`▼하방%`(`--st-hot`)·앵커명. 하방여지<2%=「지지선 근접」, 표시 9.9x 캡. 신규 토큰 없음(기능색 재사용)·`node scripts/check-docs.mjs` 통과. 기존 표 결·`pe-tbl`·정렬 UI 불변. SimpleorNothing 지시.
- 2026-08-16 15:20 · **05 추정 리비전 트래커 종목명 1Y 주가 hover 카드.** 종목명에 `data-id`·표시 메타를 연결하고 기존 전역 `hover-chart.js`의 가격·YTD·1Y 일봉 카드와 위치 보정 로직을 그대로 재사용한다. 점선 밑줄과 도움말 커서로 hover 가능 영역을 표시하며 터치 전용 기기는 종전 동작을 유지한다. SK하이닉스(`000660`)와 BESI도 일별 시세 크론 커버리지에 포함한다. 신규 `:root` 토큰 0, gamma·리비전·정렬 산식 불변.
- 2026-08-16 09:31 · **05 추정 리비전 트래커 목표가 상승여력 막대.** `upside-bars.js` 자가 마운트로 목표가 숫자와 `여력 ±N%` 텍스트는 그대로 두고 그 아래 72×5px 가로 막대를 추가한다. 전 종목을 0~100% 동일 스케일로 비교하며 100% 초과는 막대만 최대 폭으로 캡하고 수치는 보존한다. 양수=`--st-dawn`, 음수=`--st-hot`, 트랙=`--line`의 기존 기능색을 재사용하고 신규 `:root` 토큰은 없다. `role=img`·`aria-label`로 수치 의미와 스케일을 보조한다. 데이터·행 정렬·RAER 산식은 불변. (OPS 동반)
- 2026-08-11 · **04 ④ 「칩 제조사별 Capex」 표에 인텔 행 추가(4행→5행).** 기존 `ds-mtx.plan` 표에 행 하나를 append 하고 sumrow 각주 문구를 교체한 것뿐 — **신규 `:root` 토큰·CSS 클래스 0**. 역할 배지는 파운드리 동종인 TSMC 행과 동일하게 `.ds-lb.sem` `L2·L4`, 상향 표기는 기존 `.ds-rev.up` ▲(= 이번 캡처에서 상향) 규약 그대로 재사용해 표 안에서 읽는 눈이 바뀌지 않도록 했다. `<small>` 보조 라벨(파운드리 · IDM)·셀 4칸 구조·열 너비(14/20/23/22/21%) 전부 기존 행과 동일 — 헤더·레이아웃 무변. `index.html`·`pantone.css`·`insight.css` 무편집 · TOKENS 무변 · `check-docs` 통과(토큰 25종·폰트 v1.3.9). (OPS §3·§9 동반)
- 2026-08-09 · **02 인테이크 바에 「캡처 붙여넣기」 버튼(`#insPaste`) 추가.** 포커스가 입력칸 밖이면 브라우저가 `paste` 이벤트를 안 주므로, 클릭이라는 확실한 사용자 제스처로 `navigator.clipboard.read()` 를 태우는 명시 경로를 인테이크 바(`.ins-bar`)에 둔다. 「관점 뽑기」(주 버튼)와 「비우기」 사이 = 사용 빈도 순서. **신규 `:root` 토큰·CSS 클래스 0** — 기존 보조 버튼 `.ins-btn`(비주 버튼 결·폼 14px 하한 §2·면 radius 3px §3) 그대로 재사용하고 `primary` 를 붙이지 않아 주 버튼 1개 규약을 지킨다. 드롭존(`.ins-drop`) 카피 끝에 `· 캡처는 Ctrl+V` 를 덧붙여 키 경로도 화면에 노출(문구만 · 레이아웃 무변). 실패·빈 클립보드·권한 차단 안내는 기존 상태줄 `.ins-msg` 재사용 — 새 알림 컴포넌트 만들지 않는다. `insight.css`·`index.html` 무편집 · TOKENS 무변 · `check-docs` 통과(토큰 25종·폰트 v1.3.9). (OPS §3·§6-7·§9 동반)
- 2026-08-03 21:30 · **본문 복원.** 8/2 `6f6782a` 커밋에서 셸 변수가 확장되지 않아 문서 전체가 `$(cat …)` 리터럴 1줄(540 B)로 대체된 회귀를, 마지막 정상 커밋 `2af0cf6`(111,759 B)에서 복원했다. TOKENS 자동 생성 구역·컴포넌트 규약·레퍼런스 구현 §6 전부 무변 — 복원만.
...
- 2026-07-31 14:04 · **Worker 회귀 복구·재배포.** 최신 main의 티커 UI/API 변경은 보존하고 사이트 반영·Sonnet 5 복구·회계분기 정규화 백엔드만 함수 단위 복원. UI·DOM·CSS·오류 줄 위치 변화 없음, 신규 `:root` 토큰 0 → TOKENS 구역 무변. (OPS §3·§9 동반)

- 2026-07-31 13:07 · **06 「대담 다시 굽기」 진행 패널.** 버튼의 「굽는 중…」만 보이던 상태를 기존 `#brPlayer` 위치의 패널로 확장해 4단계 카드, `N/4` 현재 상태, 1초 단위 경과시간과 무한 이동 막대를 표시한다. 모바일은 2×2 단계 배치, 모션 축소 설정에서는 막대 속도를 낮춘다. 기존 패널·선·상태색 토큰만 재사용해 신규 `:root` 토큰 0. 완료 시 패널을 기존 대담 플레이어로 교체한다. (OPS §3·§9 동반)

- 2026-07-31 12:23 · **02 「사이트 반영」 실패 피드백 명료화.** 카드 상태 줄의 기존 위치·글자 크기·색은 유지하고 영문 `claude response not json` 대신 자동 재시도 후의 한국어 원인을 표시. 신규 DOM·CSS·`:root` 토큰 0 → TOKENS 구역 무변. (OPS §3·§9 동반)

- 2026-07-31 11:43 · **02 관점 뽑기 오류 피드백 명료화.** 기존 동일 위치 `#insMsg`·동일 글자 크기·색을 유지하고, 모호한 「응답 파싱 실패」를 「AI 응답 본문 비어 있음」/「JSON 형식 오류」로 구분. 신규 DOM·CSS·`:root` 토큰 0 → TOKENS 구역 무변. (OPS §3·§9 동반)

- 2026-07-31 · **02 「사이트 반영」 교차메뉴 확장.** 기존 `.ins-apply` 버튼·`.ins-lc-*` 모달·`.ins-ap-card`/`.ins-ap-st` 상태줄을 그대로 재사용하면서 대상 카드에 `signal_log.json` 시장 맥락과 `calendar.json` 일정을 추가한다. FOMC 같은 `macro`/`calendar` narrative도 관련 보드·시장 맥락·동일 일정에 표시되며 카드별 결과 문구는 기존 반영됨/변경 없음/실패 패턴을 유지한다. **신규 DOM·CSS 클래스·`:root` 토큰·레이아웃 변경 0** — 데이터 매칭·저장 경로만 확장. (OPS §3·§9 동반)
- 2026-07-30 17:10 · **회계연도 표기를 달력 기준으로 통일.** 02 인사이트 생성·반영 결과의 기업별 FY/FQ 분기는 실제 종료일 기준 `nQyy`로 정규화하고, 회계연도 전체 수치는 단일 CY로 오인되지 않게 실제 포함 기간으로 표시한다. 기존 화면의 활성 데이터도 같은 기준으로 정리했다. 신규 DOM·CSS 클래스·`:root` 토큰·레이아웃 변경 0 — 데이터 표기와 생성 로직만 변경.
- 2026-07-30 15:27 · **02 저장 원문 글자수 제한 해제.** 새 저장분은 입력 원문 전체를 보존하며, 원문 페이지의 20,000자 상한 안내를 제거했다. 제한 해제 전에 잘린 과거 자료에만 기존 저장분 안내와 재저장 방법을 표시한다. 신규 DOM·CSS 클래스·`:root` 토큰·레이아웃 변경 0 — 저장 동작과 상태 문구만 변경. 분석 입력 120,000자 컷은 별도 규율로 유지한다. (OPS §3·§9 동반)
...
- 2026-08-08 · **03 전문가 원탁 참고자료 텍스트·캡처 이미지 붙여넣기.** 기존 `.cl-drop`을 단일 진입점으로 확장하고, 이미지 원본은 저장하지 않으며 Gemini Vision 전사 결과만 토론 자료에 첨부한다. 기존 `.cl-blk`·`.cl-chip`·`.cl-note`·`.cl-btn`과 글자 크기·면 규약을 그대로 사용한다.
...
- 2026-07-30 · **02 인사이트 「사이트 연관」 내부 근거 표시.** 관점 생성 전에 01 시장 모니터링·02 기존 채택 관점·03 전문가 원탁·04 시장/실적 전망·05 비중조절 컨텍스트를 자동 대조하고, 실제 사용된 근거만 claim `siteRefs[]`로 반환·저장한다. 추출 행과 채택 관점에서 기존 검증문 컴포넌트 `.ins-vf`를 재사용해 `메뉴 · 항목 (기준일) : 근거`를 표시한다. 신규 클래스·`:root` 토큰·CSS 0 → TOKENS 무변·check-docs 무영향. 기존 「반영하기」·라이프사이클·시그널 중첩 순서는 유지한다. (OPS §3·§9 동반)
...
- 2026-07-18 14:35 · **05 비중조절 추정 리비전 트래커에 「기대수익 점수」 컬럼(`raer.js` 자가 마운트 · PR #423).** SimpleorNothing 지시. `#probEst .pe-tbl`에 위험조정 기대수익(RAER = 여력×실현확률÷리스크, 15행 상대 0–100) 컬럼을 종목 다음(2번째)에 주입, 점수 내림차순 재정렬, **현금 행**을 종목으로 추가. index.html·worker.js·pantone.css 무편집 — 이미 로드되는 `changelog.js` 부트스트랩이 `loadRaer()`로 `<script defer src="/raer.js">` 주입(flags.js·aisd.js 패턴). CSS는 JS 주입(`.raer-sc`/`.raer-bar` — `pe-bar` 결 계승·면/부표 radius 규약 준수). 점수 tier 색은 기능색 재사용(≥60 `--st-dawn`·≥40 `--st-mature`·<40 `--st-hot`·현금 `--faint` — insight 등급·aisd 판정 선례). **신규 `:root` 토큰 0** → TOKENS 무변·check-docs 통과. 값 15px·메타 12px(§2 하한). MutationObserver로 트래커 재렌더 시 컬럼 유지. narrative≠numbers·관측 휴리스틱(예측·투자권유 아님). (OPS §3·§9 동반)
