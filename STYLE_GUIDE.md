...
| 차트 값 오버레이 | `dr()`가 캔버스 **상단 여백(y<18)에 직접 그린다** — 처음값(좌·시작점 위, `#868e96`) / 마지막값(중앙·`bold`·라인 색) / 등락%·N일(우, 라인 색). 캔버스 내부라 그래프에 붙어 표시되고 Ctrl+휠·호버 시 창과 함께 동기 갱신. 라인 색 = 상승 `--st-hot` / 하락 `--st-accel`. 하단 좌·우엔 시작·끝 날짜(`fd()`). **Ctrl+휠 안내 텍스트는 두지 않는다**(기능은 유지) |
