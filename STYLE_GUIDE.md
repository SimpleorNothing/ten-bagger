...
> 2026-08-30 08:36 · **02 기업분석 전략 프레임 우측 분기 재무표.** 기존 양면 프레임을 유지하고 우측 `.ca-frame-side` 안에 3열 소형 표(`분기 / 매출 / 영업이익`)를 배치한다. 실제와 전망은 별도 신규 색상 없이 기존 `--dawn` 및 옅은 배경으로 구분하고, 표 본문·단위·CY 대응분기·산식 주석은 메타 하한 12px을 지킨다. 900px 이하에서는 기존 단일열 전환을 그대로 적용하며 신규 `:root` 토큰은 추가하지 않는다.
