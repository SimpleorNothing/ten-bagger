...
- 2026-07-12 17:45 · **WTI 유가 카드 로딩 고착 수리.** `loadWti`가 응답을 `j.series`로 읽었으나 worker `/api/wti` 출력은 `{source,points:[["YYYY-MM-DD",close]]}` — 항상 0건 → 카드가 「로딩…」에 영구 고착. `j.points||j.series`로 읽도록 수정, 대기·실패 시엔 렌즈(l1) 유지한 「유가 수집 대기」 상태로 폴백(빈 화면 금지, §6-6). #288이 패치 파일만 커밋하고 index.html 미적용 → 본 PR에서 실제 반영. (전일대비 배지는 `card()` 경유로 자동 표시)
