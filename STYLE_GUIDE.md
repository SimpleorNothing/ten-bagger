...
- 2026-08-01 16:20 · **03 전문가 원탁 중복 `council.json SoT` 직접 로드 제거.** 후속 병합에서 `index.html` 하단 직접 스크립트가 다시 유입돼 중복 결과·이력 블록이 표시된 것을 제거했다. 기본 토론 이력 한 곳과 전체 누적 저장은 유지한다. 신규 토큰·CSS 0. (OPS §3 동반)

- 2026-08-01 08:30 · **03 전문가 원탁 이력 보관 한도 제거.** 서버의 20건 슬라이스를 없애 삭제 전까지 모든 토론 이력을 누적 보관하며, 이력 안내도 전체 보관으로 정정했다. 기존 이력 UI·삭제/음성 재생·토큰은 불변. (OPS §3 동반)

- 2026-08-01 08:25 · **03 전문가 원탁 중복 블록 직접 로드 제거.** Worker 주입 제거만으로는 `index.html` 하단의 직접 `council-sot.js` 로드가 남아 `council.json SoT` 결과·중복 이력이 계속 표시됐다. 해당 로드를 삭제해 기본 토론 이력 한 곳만 보이도록 마무리했다. 신규 토큰·CSS 0. (OPS §3 동반)

- 2026-08-01 08:10 · **03 전문가 원탁 중복 `council.json SoT` 블록 제거·이력 누적 복원.** `council-sot.js`를 로드하지 않아 상단 결과와 중복 이력 블록을 제거하고, 기본 토론 이력은 최신순으로 건수 제한 없이 보관한다. 기존 `.cl-*` UI·삭제/음성 재생·토큰은 그대로이며 신규 토큰·CSS 0. (OPS §3 동반)
