...
- 2026-08-01 08:10 · **03 전문가 원탁 중복 `council.json SoT` 블록 제거·이력 누적 복원.** `council-sot.js`를 로드하지 않아 상단 결과와 중복 이력 블록을 제거하고, 기본 토론 이력만 최신순으로 최대 20건 보관한다. 기존 `.cl-*` UI·삭제/음성 재생·토큰은 그대로이며 신규 토큰·CSS 0. (OPS §3 동반)
