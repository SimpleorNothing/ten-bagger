...
> 2026-08-30 16:43 · **02 기업분석 전략 프레임 우측 분기 재무 세로 막대그래프.** 기존 양면 프레임을 유지하고 우측 `.ca-frame-side` 안에 분기별 두 막대(`매출 / 영업이익`)를 나란히 배치한다. 상단은 색상 범례와 달력형 `분기별` 표시, 본문은 수평 격자·우측 GAAP·$B 눈금·압축 X축 라벨로 첨부 형태를 따른다. 두 계열은 기존 `--st-accel`·`--st-dawn`을 재사용하고, 실적은 단색·전망은 저채도 점선으로 구분한다. 산식 주석·Q4E `미공시` 자리표시자는 메타 하한 12px을 지키며 유지하고, 900px 이하 기존 단일열 전환과 620px 차트 축소를 적용한다. 신규 `:root` 토큰은 추가하지 않는다.
