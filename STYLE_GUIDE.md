...
| 차트 값 오버레이 | `dr()`가 캔버스 내부에 직접 그린다. **처음·마지막 값은 '그래프 아래' 하단 눈금줄(`H−4`)**에 배치 — 끝점 위에 얹으면 라인과 겹쳐 안 보였음(사용자 피드백). 하단 바깥=날짜(`fd()`·`#868e96`) · 안쪽=값(`fv()`·`bold`·라인 색). **기간별 증감률(%·N일)만 좌상단**(라인 색). **수치 포맷 `fv()` = 10 미만 소수 1자리 / 10 이상 정수(콤마)** — 처음·마지막·호버·평단 공통. Ctrl+휠·호버 시 창과 함께 동기 갱신. 라인 색 = 상승 `--st-hot` / 하락 `--st-accel`. **Ctrl+휠 안내 텍스트는 두지 않는다**(기능은 유지) |
