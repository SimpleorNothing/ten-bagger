...
> 최종 갱신: 2026-07-12 · v3 (레퍼런스 구현 §6·신규 메뉴 체크리스트 §7 신설)
...
| `.stk-body` | 좌 `.stk-left`(62%) / 우 `.stk-chart`(36%, 좌측 보더). `@media(max-width:700px)` → 세로 스택, 캔버스 190→150px |
...
11. UI/CSS를 건드렸으면 **`node scripts/check-docs.mjs` 통과** + 이 문서 갱신 이력 한 줄.
