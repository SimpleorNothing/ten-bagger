...
| 차트 값 오버레이 | `dr()`가 캔버스 내부에 직접 그린다. **처음값·마지막값은 각 끝점 y 위**(`Y(v[0])`/`Y(v[k-1])`−4px, 클램프로 캔버스·증감률과 겹침 방지)에 붙여 표기 — 처음값=좌·`#868e96` / 마지막값=우·`bold`·라인 색(#296 `.spv` 끝점 오버레이와 동일 개념). **기간별 증감률(%·N일)은 좌상단**(라인 색). Ctrl+휠·호버 시 창과 함께 동기 갱신. 라인 색 = 상승 `--st-hot` / 하락 `--st-accel`. 하단 좌·우엔 시작·끝 날짜(`fd()`). **Ctrl+휠 안내 텍스트는 두지 않는다**(기능은 유지) |
