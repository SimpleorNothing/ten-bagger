...
2026-09-12 실제 운영 호출에서는 종합매매와 개인형IRP는 계좌목록에 반환됐지만 DC는 `/n2/acctinfo` 자체에 반환되지 않았다. 따라서 현재 DC는 코드 필터 문제가 아니라 NHPLUG 제공 범위 밖으로 판정하고 수동 스냅샷을 사용한다.
...
5. DC 수동 스냅샷의 현금이 확인되지 않은 경우 보유 ETF 평가금액 합계를 DC 전체자산으로 오인하지 않는다.
...
- 수동 DC 기준일이 2026-09-12인지
- 9개 ETF 평가금액 합계가 431,206,120원과 일치하는지
- 현금 미확인 상태가 명시되어 있는지
