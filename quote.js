...
(function () {
  ...
  function mount() {
    var v = document.getElementById('v-market'); if (!v) return;
    removeStrayLiteralNewlines(v);
    if (document.getElementById(HOST_ID)) return;
    css(); var host = document.createElement('div'); host.id = HOST_ID; host.title = '누르면 다른 명언'; v.insertBefore(host, v.firstChild); render(host);
    host.addEventListener('click', function () { render(host); });
    fetch('/signals.json?t=' + Date.now(), { cache: 'no-store' }).then(function (r) { return r.ok ? r.json() : null; }).then(function (j) { var rg = regime(j); if (!rg) return; ST.sig = j; if (rg !== ST.rg) { ST.rg = rg; ST.last = -1; } render(host); }).catch(function () {});
  }
