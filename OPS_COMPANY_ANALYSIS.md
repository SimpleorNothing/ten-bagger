...
증권사 목표가 변경, 하루 주가 등락, 출처 불명 수치는 기업 전략·현황의 사실 데이터로 넣지 않는다. 회사 공식 실적 원문이 존재하면 MarketWatch 등 2차 보도를 기본 출처로 두지 않는다.

## 3. 공통 company 데이터 스키마 계약

모든 `*/data.json`은 하나의 공통 스키마만 사용한다. 기업별 별칭 필드를 허용하지 않는다.

- `financials[].growth`: YoY. `yoy` 금지
- `financials[].opMargin`: GAAP 영업이익률. 자료가 없을 때만 `null`
- `financials[].netMargin`: GAAP 순이익률. 자료가 없을 때만 `null`
- `risks[].detail`: 위험 본문. `text` 금지
- `sources[].label`: 출처 표시명. `name` 금지
...
`company.js` 렌더러가 새 필드 별칭을 수용하도록 확장해서는 안 된다. 데이터가 렌더러 계약에 맞춰야 한다. `scripts/validate-company-analysis.mjs`가 Marvell과 Lumentum 모두에 대해 이 계약을 검사하며 위반 시 PR을 실패시킨다.
...
- 일회성 회계항목이 GAAP 순이익률을 크게 왜곡해도 값을 비워두지 않는다. GAAP 값을 표시하고 원인·금액을 note로 설명하며, 필요하면 non-GAAP 정상화 수치를 별도로 병기한다.
...
- Lumentum FY27 Q1E 매출 `$1.250B`는 회사 가이던스 `$1.225~1.275B`의 중간값이며 GAAP 영업이익은 미공시다. FY27 Q2E는 회사 가이던스가 없어 매출·GAAP 영업이익을 임의 추정하지 않는다.
...
- 사업 매각·중단영업·채무소멸손실처럼 GAAP 마진·순이익 해석을 왜곡하는 일회성 사건
...
- 모든 수정 JSON parse 성공
- `company.js`, `company-clock.js`, `company-patch.js`, `site-change-live.js` JavaScript syntax 통과
- Marvell/Lumentum `data.json`이 공통 스키마 계약을 준수
...
- FY27 Q1: 매출 $1.225~1.275B, 비GAAP OPM 39.5~40.5%, 비GAAP EPS $4.05~4.35 달성 여부
- FY26 Q4 이후: 1.6T transceiver, 200G EML·CW/UHP laser 출하와 비GAAP GM 50%대 지속 여부
- OCS: FY26 Q2 $400M 초과 backlog의 출하·Systems 매출 전환 속도
- CPO/NPO: CY2027 H1 납품 예정 수억달러 주문의 qualification·매출 인식
- NVIDIA: 비독점 수십억달러 purchase commitment 소진 속도와 Coherent 대비 실제 content share
- Greensboro: Qorvo 기존 fab retrofit, 인력 전환, 6-inch InP qualification·수율, CY2028 mid ramp 준수 여부
- 자본구조: 잔존 전환사채, 추가 교환·희석, 완전희석주식수, CAPEX·FCF·ROIC
